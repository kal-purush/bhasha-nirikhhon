...
non_latin_languages = [
    Language.ARABIC,
    Language.ARMENIAN,
    Language.AZERBAIJANI,  # Sometimes non-Latin
    Language.BELARUSIAN,
    Language.BENGALI,
    Language.BULGARIAN,
    Language.CHINESE,
    ...
    Language.HINDI,
    Language.JAPANESE,
    Language.KAZAKH,        # Often Cyrillic
    Language.KOREAN,
    Language.MACEDONIAN,
    Language.MARATHI,
    Language.MONGOLIAN,
    ...
    Language.PUNJABI,
    Language.RUSSIAN,
    Language.SERBIAN,       # Cyrillic is official
    Language.TAMIL,
    Language.TELUGU,
    Language.THAI,
    Language.UKRAINIAN,
...
block_to_lingua = {
    ...
    "Arabic Supplement": [Language.ARABIC],
    "Arabic Extended-A": [Language.ARABIC],
    "Armenian": [Language.ARMENIAN],
    "Bengali": [Language.BENGALI],
    "Cyrillic": [Language.RUSSIAN, Language.BELARUSIAN, Language.BULGARIAN, Language.UKRAINIAN, Language.KAZAKH, Language.MACEDONIAN, Language.SERBIAN, Language.MONGOLIAN],
    "Cyrillic Supplement": [Language.RUSSIAN],
    "Cyrillic Extended-A": [Language.RUSSIAN],
    ...
    "Hangul Syllables": [Language.KOREAN],
    "Hangul Jamo": [Language.KOREAN],
    "Hangul Compatibility Jamo": [Language.KOREAN],
    "Hangul Jamo Extended-A": [Language.KOREAN],
    "Hangul Jamo Extended-B": [Language.KOREAN],
    "Hiragana": [Language.JAPANESE],
    "Katakana": [Language.JAPANESE],
    "Katakana Phonetic Extensions": [Language.JAPANESE],
    "CJK Unified Ideographs": [Language.CHINESE, Language.JAPANESE],
    "CJK Unified Ideographs Extension A": [Language.CHINESE, Language.JAPANESE],
    "CJK Symbols and Punctuation": [Language.CHINESE, Language.JAPANESE, Language.KOREAN],
    "CJK Radicals Supplement": [Language.CHINESE],
    "CJK Combined (Chinese, Japanese, Korean)": [Language.CHINESE, Language.JAPANESE, Language.KOREAN],
    "Kangxi Radicals": [Language.CHINESE],
    "Bopomofo": [Language.CHINESE],
    "Bopomofo Extended": [Language.CHINESE],
    "CJK Strokes": [Language.CHINESE],
    "Japanese Kana": [Language.JAPANESE],  # shorthand
    "Tamil": [Language.TAMIL],
    "Telugu": [Language.TELUGU],
    "Thai": [Language.THAI],
    "Mongolian": [Language.MONGOLIAN],
}
# Function to query the block-to-language mapping
def get_lingua_languages_for_block(block_name: str):
    return block_to_lingua.get(block_name, [])
...
def process_hour(hour, year, month, day, download_dir, ids):
    ...
    try:
                                ...
                                if item_id and item_id in ids:
                                    ...
                                    if scripts:
                                        ...
                                        if len(scripts)==1:
                                            ...
                                            #       f"for message ID: {item_id}, {scripts}\n"
                                            #       f"{message}\n\n\n")
                                            has_detected = False
                                            if detection:
                                        ...
                                        #                 script = scripts[0] if 'Latin' not in scripts[0] else scripts[1]
                                        #                 if 'Latin' not in script:
                                        #                         languages_list = get_lingua_languages_for_block(script)
                                        #                         new_detector = LanguageDetectorBuilder.from_languages(*languages_list).with_preloaded_language_models().with_minimum_relative_distance(0.99).build()
                                        #                         non_english_message_part = extract_non_english(message)
                                        #                         new_detection = new_detector.compute_language_confidence_values(non_english_message_part)
                                        #                         # print(f"Detected language: {new_detection} \n"
                                        #                         #     f"for message ID: {item_id}, {scripts}\n"
                                        #                         #     f"{non_english_message_part}\n\n\n")
                                        #                         if new_detection:
                                        #                             detected_lang = new_detection[0]
                                        #                             if detected_lang.value >= 0.9:
                                        #                                 new_lang = detected_lang.language.name.lower()
                                        #                                 local_counts[new_lang] += 1
                                        #                                 has_detected = True
                                        #                             else:
                                        #                                 has_detected = False
                                        #                 else:
                                        ...
                                        #         could_not_process.append(item_id)
                                            ...
                                            if detection:
                                                for result in detection:
                                                    confidence_value = detector.compute_language_confidence(message[result.start_index:result.end_index], result.language)
                                                    # print(f"{message[result.start_index:result.end_index]} ====> {confidence_value}, {result.language.name}")
                                                    if confidence_value>0.9:
                                                        if result.language.name.lower() not in languages:
                                                            local_counts[result.language.name.lower()] += 1
                                                            has_detected = True
                                                            languages.add(result.language.name.lower())
                                            
                                            if not has_detected:
                                                could_not_process.append(item_id)
                                            
                                            # print(f"Detected language: {languages} for message ID: {item_id}, {scripts}\n{message}")
        ...
        raise
        return None
    return local_counts, could_not_process
...
if __name__ == "__main__":
    ...
    for day in range(1, 32):
        try:
            ...
            with Pool(processes=min(1, 24)) as pool:
                for result in tqdm(pool.imap_unordered(func, range(24)), total=24, desc="Processing hours"):
                    if result:
                        local_counts, could_not_process = result
                        # # save messages to a file
                        # with open(f"messages_{day_str}.txt", 'a', encoding='utf-8') as f:
