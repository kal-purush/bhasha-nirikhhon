...
def process_hour(hour, year, month, day, download_dir, ids, df, message_dict):
                    ...
                    if event_type in {"IssuesEvent", "IssueCommentEvent","PullRequestEvent", "PullRequestReviewEvent", "PullRequestReviewCommentEvent"}:
                        if 'bot' not in event.get("actor", {}).get("login", ""):
                            ...
                            if event_type == "IssuesEvent":
                                ...
                                actor_id = issue.get("user", {}).get("id", "")
                                actor_name = issue.get("user", {}).get("login", "")
                                ...
                                actor_id = pr.get("user", {}).get("id", "")
                                actor_name = pr.get("user", {}).get("login", "")
                                ...
                                html_url = pr.get("html_url","")
                                item_id = pr.get("id", None)
                                actor_id = issue.get("pr", {}).get("id", "")
                                actor_name = issue.get("pr", {}).get("login", "")
                                if 'bot' in pr.get("user", {}).get("login", ""):
                                    shoudl_process = False
                            if shoudl_process:
                                ...
                                if language:
                                    for lang in language:
                                        messages.append((lang, event_type, message, item_id, html_url, actor_id, actor_name))
...
if __name__ == "__main__":
    ...
    if not os.path.exists("test_only"):
        os.makedirs("test_only")
    for day in range(1, 32):
        try:
            ...
            df = pd.read_csv(f"resul_files/messages_with_languages_{year}_{month:02d}_{day:02d}.csv", engine='python', on_bad_lines='skip')
            ...
            func = partial(process_hour, year=year, month=month, day=day, download_dir=download_dir, ids=ids, df=df, message_dict=message_dict)
            ...
            # df_counts.to_csv(f"test_only/script_count_{year}_{month:02d}_{day:02d}.csv", index=False)
            # print(f"Counts saved to script_count_{year}_{month:02d}_{day:02d}.csv")

            df_messages = pd.DataFrame(all_detected_messages, columns=["language", "event_type", "message", "id", "html_url", "actor_id", "actor_name"])

            df_messages.to_csv(f"test_only/messages_with_languages_{year}_{month:02d}_{day:02d}.csv", index=False)
    ...
    # # print("Could not process IDs:", len(ids_could_not_process))
    # with open(f"test_only/could_not_process_ids_{year}_{month:02d}.txt", "w") as f:
