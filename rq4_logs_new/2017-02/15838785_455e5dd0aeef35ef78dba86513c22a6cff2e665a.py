from rdflib_zodb import ZODB
from rdflib.plugins.stores.concurrent import ResponsibleGenerator
from threading import Lock


class ConcurrentStoreMixin(object):
    def __init__(self):
    ...
    def add(self, triple):
        ...
        if self.__visit_count == 0:
            super(ConcurrentStoreMixin, self).add((s, p, o))
        else:
    ...
    def remove(self, triple):
        (s, p, o) = triple
        if self.__visit_count == 0:
            super(ConcurrentStoreMixin, self).remove((s, p, o))
        else:
            self.__pending_removes.append((s, p, o))

    def triples(self, triple):
        (su, pr, ob) = triple
        g = super(ConcurrentStoreMixin, self).triples((su, pr, ob))
...
class ConcurrentZODBStore(ConcurrentStoreMixin, ZODB.ZODBStore):
    pass