...
class Blackjack:
    ...
    async def bet(self, ctx, bet: int):
        ...
        if self.bot.get_cog('Economy').bank.can_spend(player, bet) and self.game_state == "pregame":
            if bet < self.settings["BLACKJACK_MIN"] or (bet > self.settings["BLACKJACK_MAX"] and self.settings["BLACKJACK_MAX_ENABLED"]):
                ...
                await self.bot.say("{0} has a balance of: {1}".format(player.mention, self.bot.get_cog('Economy').bank.get_balance(player)))
    ...
    async def hit(self, ctx):
        ...
        if self.game_state == "game" and self.players[player]["hand"][curr_hand]["standing"] == False:
            ...
            if self.settings["BLACKJACK_IMAGES_ENABLED"]:
                await self.show_hand(player, curr_hand)
    ...
    async def blackjack_game(self, ctx):
        while self.game_state != "null":
            ...
            if self.game_state == "drawing":
                ...
                if self.settings["BLACKJACK_IMAGES_ENABLED"]:
                    await self.show_hand(self.bot, curr_hand)
            ...
            if self.game_state == "endgame":
                ...
                if self.game_state == "pregame":
                    for player in self.players:
                        if player != self.bot:
                            await self.bot.say("{0} now has a balance of: {1}".format(player.mention, self.bot.get_cog('Economy').bank.get_balance(player)))
    ...
    async def show_hand(self,player,curr_hand, firstDeal:bool=None):
        cards = self.players[player]["hand"][curr_hand]["card"]
        box1 = (0,0,56,82)
        box2 = (56,0,112,82)
        count = 0
        ...
        hand.close()
        if player is self.bot:
            await self.bot.upload("data\\blackjack\\hand\\dealer_hand.png")
        else:
            await self.bot.upload("data\\blackjack\\hand\\{}_hand.png".format(player.name))
