...
public class EnhanceHostRoutingFilter extends ZuulFilter {

	private static final Log log = LogFactory.getLog(EnhanceHostRoutingFilter.class);

	private final Timer connectionManagerTimer = new Timer(
            "EnhanceHostRoutingFilter.connectionManagerTimer", true);
	...
	public void onPropertyChange(EnvironmentChangeEvent event) {
		...
		if (createNewClient) {
			try {
				EnhanceHostRoutingFilter.this.httpClient.close();
			}
			catch (IOException ex) {
				log.error("error closing client", ex);
			}
			EnhanceHostRoutingFilter.this.httpClient = newClient();
		}
	}

	public EnhanceHostRoutingFilter(ProxyRequestHelper helper, ZuulProperties properties) {
		this.helper = helper;
	...
	private void initialize() {
		...
		this.connectionManagerTimer.schedule(new TimerTask() {
			@Override
			public void run() {
				if (EnhanceHostRoutingFilter.this.connectionManager == null) {
					return;
				}
				EnhanceHostRoutingFilter.this.connectionManager.closeExpiredConnections();
			}
		}, 30000, 5000);
	}
