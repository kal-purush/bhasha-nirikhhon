...
namespace Touryo.Infrastructure.Public.Db
{
    ...
    public class DamManagedOdp : BaseDam
    {
        #region クラス変数
        ...
        private void PreExecQuery()
        {
            if (this.IsDPQ)
            {
                ...
                this._cmd.CommandText = this.Convert(DamManagedOdp._paramSign);
                ...
                foreach (string paramName in this._parameter.Keys)
                {
                    ...
                    //if (this._cmd.CommandText.IndexOf(DamManagedOdp._paramSign + paramName) == -1)
                    //{
                    //    continue; // スキップする。
                    //}
