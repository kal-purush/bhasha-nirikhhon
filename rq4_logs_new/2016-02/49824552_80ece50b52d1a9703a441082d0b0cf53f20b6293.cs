...
/* This class is linked to a gameObject. It keeps track of all the sounds to be played for this game object.
*  Each sound is a game object, with an audioclip attached to it (because only one audioclip is allowed per object).
*/
public class AudioElement {
    ...
    // TODO the check on volume between 0 and 1 should be here !
    public AudioElement(List< AudioClip > sounds, List< float > volumes, string id, Transform parentTransform) {
        ...
        // Attach the sound object to the parent object so that it moves along the object for which it should produce sound
        if (parentTransform) 
