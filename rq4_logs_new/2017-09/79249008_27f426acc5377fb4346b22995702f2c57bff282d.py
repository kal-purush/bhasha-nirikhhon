...
def main(unused_argv):
  ...
  for line in input_fo:
      if line != '\n':
          ...
          temp_data = []
          for i in range(16):
              temp_data.append(np.float32(temp[i]))
  ...
  train_data = np.asarray(data[:7000])
  train_labels = np.asarray(label[:7000])
  eval_data = np.asarray(data[7000:])
  eval_labels = np.asarray(label[7000:])
