...
var server = null;
var uv4lserver = null;
var raspidispserver = "http://" + window.location.hostname + ":9080" + "/stream/webrtc";
if(window.location.protocol === 'http:') {
	server = "http://" + window.location.hostname + ":8088/janus";
	uv4lserver = "http://" + window.location.hostname + ":8080";
} else {
	server = "https://" + window.location.hostname + ":8889/janus";
	uv4lserver = "https://" + window.location.hostname + ":8080";
}
						...
						janus.attach(
							{
								...
								success: function(pluginHandle) {
									...
									$('#startrpi').click(startRpi);
									$('#stoprpi').click(stopRpi);
									$('#usernamerpi').focus();
...
function checkEnterRpi(field, event) {
	var theCode = event.keyCode ? event.keyCode : event.which ? event.which : event.charCode;
	if(theCode == 13) {
		startRpi();
		return false;
...
function startRpi() {
	if($('#usernamerpi').length === 0) {
		// Create fields to register
		$('#startrpi').click(startRpi);
		$('#stoprpi').click(stopRpi);
		$('#usernamerpi').focus();
	} else {
		var usernamerpi = $('#usernamerpi').val();
		if(/[^a-zA-Z0-9]/.test(usernamerpi)) {
			return;
		}
		...
		var url = uv4lserver + "/janus?gateway_url=https://localhost:8889&gateway_root=/janus&room=1234&room_pin=&username=" + usernamerpi + "&token=&proxy_host=&proxy_port=80&proxy_password=&proxy_bypass=&publish=1&vformat=60&hw_vcodec=0&subscribe=1&reconnect=1&action=Start";
...
function stopRpi() {
	var xmlHttp = new XMLHttpRequest();
	var url = uv4lserver + "/janus?action=Stop";
	xmlHttp.open( "GET", url, true ); // false for synchronous request
	xmlHttp.send(null);
}

function remoteRpi() {
	var url = uv4lserver + '/janus';
