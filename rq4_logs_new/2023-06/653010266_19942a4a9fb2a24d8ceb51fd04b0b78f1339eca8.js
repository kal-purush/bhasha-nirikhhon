/*
 * @Author: zhanphty
 * @Desc: 阿里云盘自动签到，环境变量创建多个 ALI_REFRESH_TOKEN 来同时多个账号自动签到
 */
...
// 阿里云盘refreshToken环境变量名
const aliRefreshToken = "ALI_REFRESH_TOKEN"
...
async function getRefreshToken() {
  ...
  let refreshToken = process.env[aliRefreshToken] || []
  try {
    instance && (refreshToken = await getEnv(instance, aliRefreshToken))
  } catch (e) {}
  ...
  if (!refreshTokenArray.length) {
    console.log(
      `未获取到环境变量：${aliRefreshToken}, 请在“环境变量” -> “新增变量”增加 ${aliRefreshToken} 的变量`
    )
    process.exit(1)
  }
