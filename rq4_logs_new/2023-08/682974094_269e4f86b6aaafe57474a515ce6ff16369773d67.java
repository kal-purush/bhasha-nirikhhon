...
public class ClientHandler implements Runnable {
    ...
    public void run() {
            ...
            out.println("Please log in using the command: 'login  username  password' (e.g login  allan21  password1111)");
            String inputLine;
    ...
    private void handleLogin(String[] command) throws SQLException, IOException {
            ...
            if (isAuthenticated) {
                ...
                if (memberNumber != null) {
                    ...
                    out.println("We're glad to have you here at Uprise sacco. Remember that your financial well-being is our priority.");
                    out.println("Feel free to explore your account details, manage transactions, and access our range of services.");
                ...
                if(memberDetails.length != 2){
                    ...
                    // Check if there's a match for member number and telephone number in the database
                       ...
                       if (loginCommand.length == 3 && loginCommand[0].equalsIgnoreCase("login")) {
                           ...
                           if (isLoginValid) {
                            ...
                            if (memberNo != null) {
                                ...
                                out.println("Welcome back, " + loginUsername + "!                                          Date: "+ LocalDate.now());
                                out.println("We're glad to have you here at Uprise sacco. Remember that your financial well-being is our priority.");
                                out.println("Feel free to explore your account details, manage transactions, and access our range of services.");
                               ...
                               out.println("Invalid login credentials. Please return another day with this reference number: " + referenceNumber);
                           }
                       } else {
                           // Invalid login format
                           out.println("Invalid login format. Please use the following format: login username randomPassword");
                       }
                   } else {
                             // No match found, generate a reference number for the user to follow up later
                            String referenceNumber = generateReferenceNumber();
                           // out.println("No match found for the provided member number and telephone number.");
                            out.println("No match found for the provided member number and telephone number. Please return another day with this reference number: " + referenceNumber);
                            System.out.println("No match found for the provided member number and telephone number. Please return another day with this reference number: " + referenceNumber);
        ...
        try {
            ...
            while (true) {
                ...
                if (inputLine.isEmpty()) {
                    out.println("Hey there, " + saccoMember.getUsername() + ". It looks like you entered an empty choice.");
                    out.flush();
                } else {
    ...
    private void handleDeposit(String[] command) {
        if (command.length != 4) {
            ...
            try {
                ...
                if (database.isValidReceiptNumber(receiptNumber, amount, dateDeposited, saccoMember.getMemberNumber())) {
                     out.println("Dear "+ saccoMember.getUsername() +  ",");
                     out.println("We're pleased to inform you that your deposit of UGX" + amount + " has been successfully recorded and credited to your account.");
                     out.println("Your commitment to saving with us is greatly appreciated."); 
                     out.println("If you have any questions or need further assistance, please don't hesitate to reach out to our Customer Service Team at +256771895380 or uprizesacco21@gmail.com");
                     out.println("Thank you for being a valued member of Uprise.");
                     out.println(" ");
                } else {
                    //out.println("Invalid receipt number or receipt not yet uploaded. Please check later after '30 minutes'. Thank you.");
                    out.println("Dear "+ saccoMember.getUsername() +  ",");
                    out.println("We hope this finds you well.");
                    out.println("We wanted to inform you that we have not yet received the deposit upload for receipt " + receiptNumber + " in your Uprise account.");
                    out.println("If you have already made the deposit, please allow 24 hours for the transaction to reflect in our records.");
            ...
            try {
                ...
                if (saccoMember.hasPendingLoanApplications()) {
                    ...
                    if (database.insertLoanApplication(loanApplication)) {
                        out.println("Dear " + saccoMember.getUsername() + ",                                            "+ LocalDate.now());
                        out.println("Congratulations! your loan application has been successfully submitted. We appreciate your interest in our financial services.");
                        ...
                        if (accumulatedRequestsCount >= LOAN_REQUEST_THRESHOLD) {
                            ...
                            System.out.println("The loan applications will be processed once enough applications are received.");
                        }
                    } else {
    ...
    private void handleLoanRequestStatus(String[] command) throws SQLException {
        ...
        if (loanStatus != null) {
            ...
            //out.println("Hey " + username + ", your Loan status for application number " + applicationNumber + " is: " + loanStatus);
            out.println("Dear " + username + ",                       Date: " + LocalDate.now());
            out.println("Your loan application of "+ applicationNumber + " is currently under review  ("+ loanStatus +") . We appreciate your patience and will notify you once a decision is made.");
            ...
            if (loanStatus.equalsIgnoreCase("approved")) {
                ...
                out.println("We regret to inform you that your loan application " + applicationNumber + " has been declined.");
                out.println("If you have any questions or would like further details, please reach out to our Customer Service team at +256771895380 or uprisesacco21@gmail.com");
    ...
    private void handleLoanDecision(String applicationNumber) throws SQLException {
        ...
        if (decision.equalsIgnoreCase("accept")) {
            if (database.acceptLoanApplication(applicationNumber)) {
                LoanApplication acceptedLoanApplication = database.getLoanApplicationByApplicationNumber(applicationNumber);
                ...
                if (acceptedLoanApplication != null && processAcceptedLoanApplication(acceptedLoanApplication)) {                    
                  LoanApplication processedLoan = database.getProcessedLoanDetails(applicationNumber);
                    ...
                    out.println();
                    out.println("Installment Amount: UGX" + processedLoan.getExpectedInstallments() + " (principal + interest)");
                    out.println();
                    ...
                    out.println("The approved loan amount of UGX" + processedLoan.getLoanAmount() + " will be disbursed to your account on " + LocalDate.now());
                    out.println("Please review the terms of this loan carefully.");
                    out.println("Should you have any questions or require further clarification, please do not hesitate to contact our loan department at +256771895380 or uprisesacco21@gmail.com");
                    out.println("We appreciate your membership with Uprise sacco and look forward to supporting your financial journey.");
                    out.println("Thank you.");
                    out.println("................................................................");
                    out.println();
                    out.println(" ");
            ...
            if (database.rejectLoanApplication(applicationNumber)) {
                ...
                System.out.println("Hello " + saccoMember.getUsername()+  ", you have successfully rejected the loan offer.");
    ...
    private boolean processAcceptedLoanApplication(LoanApplication acceptedLoanApplication) {
        ...
        // Retrieve the amount deposited for the loan application from the loan_repayments table
        ...
        boolean isLoanInserted = database.insertLoan(
            memberId, applicationNumber, loanAmount, expectedInstallments, expectedLoanRepaymentAmount, loanPaymentPeriod, startDate, 
        ...
        if (activeLoan != null) {
            ...
            if (availableDeposit >= installmentAmount) {
                ...
                if (deductionSuccessful) {
                    ...
                    out.println("-----------------------------------------------------");
                    out.println("You have successfully repaid UGX" + String.format("%.2f", installmentAmount) + " towards your loan.");
                    out.println();
                    ...
                    System.out.println("Remaining Amount: UGX" + String.format("%.2f", activeLoan.getRemainingAmount()));
    ...
    private void handleCheckStatement(String[] command) {
        ...
        List<LoanPayment> loanPayments = database.getLoanPaymentsBetweenDates(saccoMember.getMemberNumber(), dateFrom, dateTo);
        List<Contribution> contributions = database.getContributionsBetweenDates(saccoMember.getMemberNumber(), dateFrom, dateTo);
        ...
        double memberPerformance = (loanProgress + contributionProgress) / 2; // Calculate the average of loan and contribution progress
        ...
        for (Contribution contribution : contributions) {
            out.println(contribution.getDate() + " - Amount: UGX" + String.format("%.2f", contribution.getAmount()));
        }
...
private double calculateContributionProgress(List<Contribution> contributions, LocalDate dateFrom, LocalDate dateTo) {
    // Calculate contribution progress based on total contributions and expected contributions
    ...
    // Calculate the actual total contributions made by the member within the time frame
...
private double calculateExpectedContributionAmount(LocalDate dateFrom, LocalDate dateTo) {
    // Calculate the number of contribution intervals (e.g., months) within the specified time frame
    long numberOfIntervals = ChronoUnit.MONTHS.between(dateFrom, dateTo) + 1; // +1 to include the end month

    // Get the member's expected contribution amount per interval (e.g., monthly contribution amount)
...
private double calculateTotalActualContributions(List<Contribution> contributions, LocalDate dateFrom, LocalDate dateTo) {
    // Calculate the actual total contributions made by the member within the time frame
    ...
    for (Contribution contribution : contributions) {
        ...
        if (contributionDate.isEqual(dateFrom) || contributionDate.isEqual(dateTo) || (contributionDate.isAfter(dateFrom) && contributionDate.isBefore(dateTo))) {
