...
def get_data(itration, initial, batch, method):
    """
    Generate data for plotting the performance of different sampling methods over iterations.

    Parameters:
    iteration (List[int]): Number of iterations for each method
    ...
    for ind in range(len(batch)):
        ...
        # output[f'ran_{ind + 1}'] = np.concatenate([y1, np.full(col - len(x), np.nan)])
        output[f'amb_{ind + 1}'] = np.concatenate([y2, np.full(col - len(x), np.nan)])
