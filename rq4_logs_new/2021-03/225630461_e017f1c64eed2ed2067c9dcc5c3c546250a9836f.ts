...
test('calculates "minimum" and "maximum" values', () => {
  ...
  const result = calculateLineStackedRange(data, 'auto', 'auto', [
    'amount',
    'profit',
  ]);
...
test('setup "minimum" based on provided value', () => {
  ...
  const result = calculateLineStackedRange(data, -40, 'auto', ['amount']);
...
test('setup "maximum" based on provided value', () => {
  ...
  const result = calculateLineStackedRange(data, 'auto', 100, ['amount']);
