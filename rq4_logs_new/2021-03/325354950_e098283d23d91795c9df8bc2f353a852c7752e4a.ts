...
export const checkApplicationPageParams = (applicationNumberParam: string | undefined): number => {
  // Try to parse application number from path
  if (applicationNumberParam === undefined) {
    throw new InvalidPathParametersError('Please specify an application number in the URL.');
  }
  ...
  if (isNaN(parsedApplicationNumber)) {
    throw new InvalidPathParametersError('The application number in the URL is invalid.');
  }
