...
class NewStrategy(StrategyBase):
    ...
    def start(self, broker, tick):
        ...
        self.last_candles = {tf: self.last_tick - self.time_delta \
...
order in length.")
    ...
    def _tick_tf_time_check(self, tick):
        ...
        if self.mode == 'backtest':
            ...
            raise ValueError("mode must be set to 'backtest' or 'live'")
    ...
    def _tf_time_check(self, tick, tf):
        ...
        if self.mode == 'backtest':
            ...
            (tick - self.last_ticks[tf]) >= self.timeframe_delta[tf]:
                self.last_ticks[tf] = tick
                return True
        else:
            self.tick_tf_time_check(tick, mode=mode)
    ...
    def _has_changes(self, df, tf, include_current=settings.GET_INCOMPLETE_CANDLES):
        '''
        Check if new candles came in, otherwise there is a lapse in
        trading data and no new annotations should be done.
