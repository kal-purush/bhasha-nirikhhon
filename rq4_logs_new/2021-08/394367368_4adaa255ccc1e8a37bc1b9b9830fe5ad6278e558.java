...
public class JuntarDinheiro1 {
	...
	public void dinheiro() {
		boolean x = true;
        System.out.println("\nPor favor digite seu sal�rio: R$");
        salario = menu.nextInt();
		while (x) {
	        System.out.print("\tDeseja guardar quanto do seu sal�rio?\n");
	        ...
	        if (opcao > 6 || opcao == 0){
	            System.out.print("\n\tOp��o n�o encontrado! Por favor tente novamente.\n");
	            menu.close(); 
	            break;
	        }
	        switch (opcao) {
		...
		public void infoDinheiro(int v1, int v2) {
			...
			if(v1 == 3) {
				System.out.printf("\nTerceiro m�s: R$%.2f", total);
			}
			else if (v1 == 12) {
				System.out.printf("\nUm ano: R$%.2f", total);
			}
			else if (v1 == 36) {
				System.out.printf("\nTr�s anos: R$%.2f", total);
			}
			total = reserva * v2;//6 - 24 - 60
			if(v2 == 6) {		
				System.out.printf("\nSexto m�s: R$%.2f", total);
			}
			else if(v2 == 24) {		
