...
public class unitAttackCommand extends UserCommand{
    private final ArrayList<SerializedReference<Soldier>> serializedReferenceArrayList = new ArrayList<>() ;
    private final  int x , y ;
    public unitAttackCommand(User user , ArrayList<Soldier> soldiers , int x , int y) {
        super(user);
    ...
    public String execute(Game game, GameRenderer gameRenderer) {
        ...
        String res = UnitMenuController.attackEnemy(game , soldiers , x , y) ;
        if (!res.equals("OK"))
            return res ;
        return success() ;
    }
