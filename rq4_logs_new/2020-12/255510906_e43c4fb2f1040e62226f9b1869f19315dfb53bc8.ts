...
presence.on("UpdateData", async () => {
    ...
    if (window.location.hostname == "support.something.host") {
        ...
        data.state = "Article: " + document.querySelector(
            "html > body > nav > div > div >div > div > h1").textContent;
        }
        else if (window.location.pathname.includes("/category")) {
        data.state = "Category: " + document.querySelector(
            "body > nav > div > div > div > div > span.csh-navigation-title-list-subject > span.csh-category-badge.csh-font-sans-semibold").textContent;
        }
        else {
        data.state = "Browsing";
        }
    ...
    if (window.location.hostname == "something.host") {
        ...
        if (document.querySelector("head > title").textContent == "Home || SomethingHost") {
            data.state = "Home";
        }
        else {
        data.state = document.querySelector(
            "head > title").textContent.replace('SomethingHost', '').replace('|', '');
        }
    }
