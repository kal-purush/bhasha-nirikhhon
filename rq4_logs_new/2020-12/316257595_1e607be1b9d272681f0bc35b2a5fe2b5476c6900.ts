...
import branchSubscriptionManager, {
  branchSubscriptionEventChannel,
  executeActionsChannel
} from "state/branch/sagas/subscription";
import * as actions from "state/branch/actions";

describe("branchSubscriptionManager", () => {
  const mockChannel = eventChannel(() => () => {});

  const mockTask: Task = createMockTask();

  test("should subscribe for new actions", async () => {
    const saga = branchSubscriptionManager();
    const forkedSubscribe = saga.next().value;
    const subscribeSaga = forkedSubscribe.payload.fn;
    await expectSaga(subscribeSaga)
      .provide([
        // Mock part
        [matchers.call.fn(branchSubscriptionEventChannel), mockChannel]
      ])
      .take(actions.subscribe)
      .dispatch(actions.subscribe(1))
      .call(branchSubscriptionEventChannel)
  ...
  test("should not subscribe for actions twice", async () => {
    ...
    await expectSaga(subscribeSaga)
      .provide([
        // Mock part
        [
          matchers.call.fn(branchSubscriptionEventChannel),
          eventChannel(() => () => {})
        ]
      ])
      .take(actions.subscribe)
