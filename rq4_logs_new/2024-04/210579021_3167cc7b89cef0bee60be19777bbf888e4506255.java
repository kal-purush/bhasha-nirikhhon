...
@DatabaseMetaPlugin(type = "CRATEDB", typeDescription = "CrateDB", documentationUrl = "/database/databases/cratedb.html")
...
public class CrateDbDatabaseMeta extends PostgreSqlDatabaseMeta {
    ...
    /**
     * As of the time of writing (Apr 2024), CrateDB does not natively support sequence generators like the CREATE SEQUENCE command in PostgreSQL and some other SQL databases.
     * CrateDB is built to be distributed and massively scalable.
     * Certain features found in traditional single-node SQL databases, like sequence generators,
     * are not directly translatable to a distributed system because they can become bottlenecks in a distributed environment
     * due to the need for synchronization between nodes
     */
    @Override
    ...
    /**
     * CrateDB doesn't have a direct equivalent of PostgreSQL's SERIAL data type due to its distributed nature.
     * But you can achieve a similar outcome by using generated columns that produce a unique UUID.
     * Since UUIDs cannot be sorted, they can't be incremented, as there's no concept of ordering.
     */
    @Override
    ...
    public String[] getReservedWords() {
         ...
         *   from the following query against CrateDB 5.6:
         *   SELECT word FROM pg_catalog.pg_get_keywords() WHERE catcode = 'R' ORDER BY 1
         */
