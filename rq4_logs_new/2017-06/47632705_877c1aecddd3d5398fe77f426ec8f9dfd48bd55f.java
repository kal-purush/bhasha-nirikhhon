...
public class ActionResponseTimeCalculatorSpout extends AbstractCalculator implements MeasureCalculator {
	...
	public DomainMeasure calculate(EObject domainElement, DomainMeasureDefinition definition, ToolResult toolResult, TraceSet traceSet) {
		...
		} else if (transitionInfos.size() != 2) {
			throw new RuntimeException(MessageFormat.format("Unexpected number of 'TransitionInfos' found for ''{0}''. Expected 2, but found ''{1}''", domainElement, transitionInfos.size()));
		} else {
			PlaceInfo placeInfo = placeInfos.get(0);
			//TransitionInfo transitionInfo = transitionInfos.get(0);
			TransitionInfo transitionInfo = findFirstTransitionInfoOfRuleTransition(ConstantUtils.getTransitionStormSpout(), traceSet, transitionInfos);
	...
	protected TransitionInfo findFirstTransitionInfoOfRuleTransition(String rule, TraceSet traceSet,
