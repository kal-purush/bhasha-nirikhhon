...
_, bins, _ = plt.hist(data[:, 1], bins=50, density=True, label="Empirical data", alpha=0.5)
...
plt.plot(kde_plot_X, np.exp(density), 'C0', label="KDE estimate $p(x)$")
plt.xlabel("Amplitude $x$ (mm)")
plt.ylabel("Prob density $p(x)$ ($\mathrm{mm}^{-1}$)")
plt.legend()
plt.tight_layout()
xlim = plt.xlim()
...
# Argh, not sure how to plot two different colours of histograms that are normalised to a single density.
# https://stackoverflow.com/questions/47999159/normalizing-two-histograms-in-the-same-plot
plt.hist(data[data[:, 0] == 1, 1], bins=bins, density=True, label="Empirical data, C=1", alpha=0.5)
plt.hist(data[data[:, 0] == 0, 1], bins=bins, density=True, label="Empirical data, C=0", alpha=0.5)
kde = KernelDensity(kernel='gaussian', bandwidth=0.1).fit(data[data[:, 0] == 1, 1, None])
kde_plot_X = np.linspace(0, 5.5, 500)[:, None]
density = kde.score_samples(kde_plot_X)
plt.plot(kde_plot_X, np.exp(density), 'C0', label="KDE estimate $p(x|C=1)$")
kde = KernelDensity(kernel='gaussian', bandwidth=0.1).fit(data[data[:, 0] == 0, 1, None])
kde_plot_X = np.linspace(0, 5.5, 500)[:, None]
density = kde.score_samples(kde_plot_X)
plt.plot(kde_plot_X, np.exp(density), 'C1', label="KDE estimate $p(x|C=0)$")
plt.xlabel("Amplitude $x$ (mm)")
plt.ylabel("Prob density $p(x|C)$ ($\mathrm{mm}^{-1}$)")
plt.legend()
plt.tight_layout()
plt.savefig("resonance-cond-probs.png")

plt.figure(figsize=(8, 3))
plt.plot(data[data[:, 0] == 1, 2], data[data[:, 0] == 1, 1], 'x')
plt.xlim(0, 5)
plt.xlabel("Frequency (Hz)")
plt.ylabel("Amplitude $x$ (mm)")
# plt.legend()
plt.tight_layout()
