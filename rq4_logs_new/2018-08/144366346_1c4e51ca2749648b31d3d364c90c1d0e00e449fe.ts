...
export class MapFrameComponent implements OnInit, AfterViewInit {
  ...
  setup(data) {
    ...
    this.renderer.setAttribute(
      this.mapFrame.nativeElement, 'src', `https://www.google.com/maps/embed/v1/place?key=AIzaSyDX8znfh-d4u3spGhC1GvCjq6EA1pjPovQ
&q=${data.data.address}+${data.data.city}+${data.data.country}`);
  }
