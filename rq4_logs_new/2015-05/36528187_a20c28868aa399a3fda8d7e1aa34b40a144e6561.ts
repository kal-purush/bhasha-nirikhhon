import modWabr = require('./wabr');

import WABarcodeReader = modWabr.WABarcodeReader;
import WABarcode = modWabr.WABarcode;
import WAUtils = modWabr.WAUtils;
import delReaderCallback = modWabr.delReaderCallback;

export class Test {



    serverUrl: string = "";
    authorization: string = "";
    samplesLocal: string = "";
    samplesWeb: string = "https://wabr.inliteresearch.com/SampleImages/";

    bTestDropBox: boolean = true;
    bTestSamplesLocal: boolean = true;
    bTestSamplesWeb: boolean = true;
    bTestBase64: boolean = true;
    bTestUtf8: boolean = true;
    bTestUtf8Names: boolean = true;

    // Use static to be used inside of Callback function
    static bShowDiag: boolean = false;
    static bShowBarcodes: boolean = true;



    Run(reader) {
        // Read from image in DropBox
        if (this.bTestDropBox)
            this.Read(reader, "https://www.dropbox.com/s/qcd8zfdvckwwdem/img39.pdf?dl=1");

        // this.Read local files
        if (this.bTestSamplesLocal)
            this.Read(reader, this.samplesLocal + "1d.pdf");

        // this.Read from Web-located files
        if (this.bTestSamplesWeb) {

            // Read from multiple images in a single call
            this.Read(reader, this.samplesWeb + "1d.pdf" + " | " + this.samplesWeb + "2d.pdf");

            // Read specifying type
            this.Read(reader, this.samplesWeb + "postal.pdf", "postal");

            // Read Driver License (TBR code)
            this.Read(reader, this.samplesWeb + "drvlic.ca.jpg");

            // this.Read from multi-page TIF file
            this.Read(reader, this.samplesWeb + "c39.multipage.tif");


            // this.Read specifying Target Barcode this.Reader (TBR code)
            this.Read(reader, this.samplesWeb + "I25.tbr132.jpg", "", "", 132);

            // this.Read Adobe® LiveCycle® barcodes and decompress.  (UTF-8 incorrectly displayed by echo(barcode.Text))
            this.Read(reader, this.samplesWeb + "pdf417.adobe.pdf");
        }

        // this.Read UTF8-encoded values (UTF-8 incorrectly displayed by echo(barcode.Text))
        if (this.bTestUtf8)
            this.Read(reader, this.samplesWeb + "pdf417.utf8.pdf");

        // this.Read from UTF8-named files
        if (this.bTestUtf8Names) {
            this.Read(reader, this.samplesWeb + "pdf417.ЙшзщЪфг_Russian.pdf");
            this.Read(reader, this.samplesWeb + "pdf417.بساطة لأنه الأل_Arabic.pdf");
        }

        // this.Read from Base64-encoded image
        var base64: string = "";
        if (this.bTestBase64)
            this.Read(reader, base64);

        //  Read from Base64-encoded local image file
        if (this.bTestBase64) {
            base64 = WAUtils.FileToBase64(this.samplesLocal + "2d.pdf");
            if (!WAUtils.startsWith(base64, "data:")) {
                WAUtils.printLine("ERROR: " + base64);
                return;
            }
            this.Read(reader, base64);
        }

    }




    static ShowBarcodes(barcodes: WABarcode[]) {
        if (!Test.bShowBarcodes)
            return;
        if (barcodes == null)
            return;
        for (var i = 0; i < barcodes.length; i++) {
            var barcode: WABarcode = barcodes[i];
            WAUtils.printLine();
            WAUtils.printLine("Barcode Type:" + barcode.Type + "  File:" + barcode.File + "   Page:" + barcode.Page);
            //  WAUtils.printLine("META:"); WAUtils.printLine(JSON.stringify(barcode.Meta)); // .toString());
            WAUtils.printLine(barcode.Text);
            var lng = WAUtils.dictionaryLength(barcode.Values);
            if (lng > 0) {
                WAUtils.printLine("\n VALUES:");
                for (var key in barcode.Values)
                    WAUtils.printLine(key + " : " + barcode.Values[key]);
            }
        }
    }

    Read(reader: WABarcodeReader, image: string, types?: string, dir?: string, tbr?: number) {
        var callback: delReaderCallback = this.ReaderCallback;
        reader.ReadAsync(image, callback, image, types, dir, tbr);
    }


    ReaderCallback(evnt: string, value, obj) {

        if (evnt == "error") {
            var msg = "EXCEPTION: " + value + WAUtils.signature(obj);
            WAUtils.printLine(msg);
        }
        else if (evnt == "barcodes") {
            Test.ShowBarcodes(value);
        }
        else {
            if (!Test.bShowDiag)
                return;
            if (evnt == "response") {
                var msg = "  =======>: " + value + WAUtils.signature(obj);
                WAUtils.printLine(msg);
            }
            else if (evnt == "image") {
                var msg = " ================= PROCESSING: " + WAUtils.signature(value);
                WAUtils.printLine(msg);
            }
            else if (evnt == "retry") {
                var msg = " ================= RETRY: " + value + WAUtils.signature(obj);
                WAUtils.printLine(msg);
            }
        }
    }


}