...
async def test_constructor(ownable_factory):
    ...
    # execution_info = await ownable.symbol().call()
    # assert execution_info.result == (str_to_felt("TKN"),)

    # execution_info = await ownable.symbol().call()
    # assert execution_info.result == (str_to_felt("TKN"),)

    # execution_info = await ownable.balanceOf(owner.contract_address).call()
    # assert execution_info.result.balance == uint(1000)

    execution_info = await ownable.paused().call()
    assert execution_info.result.paused == 0

    await signer.send_transaction(owner, ownable.contract_address, 'pause', [])
