// Forked:
// * Remove global definition of `global` var and `require` function, which causes duplicate definitions with `node`.
//
// TODO:
// * ReactNative nowadays no longer extends the `react` module, so the module needs to be renamed and types like
//   `Component` need to be referenced from the `React` namespace.
