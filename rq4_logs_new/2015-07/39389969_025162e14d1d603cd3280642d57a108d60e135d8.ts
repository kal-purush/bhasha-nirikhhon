class p2DebugDraw{
    ...
    private drawRevoluteJoint(joint: p2.RevoluteConstraint): void {
        ...
        this.drawCircle(anchorA, 3, this.COLOR_BLACK);
    }
    private drawGearJoint(joint: p2.GearConstraint): void {
        ...
        joint.bodyA.toWorldFrame(pA1, [20, 0]);
        joint.bodyB.toWorldFrame(pB1, [20, 0]);
        ...
        joint.bodyB.toWorldFrame(v2, [10 * (joint.bodyB.angle - joint.bodyA.angle - joint.angle), 0]);
    ...
    private drawPrismaticJoint(joint: p2.PrismaticConstraint): void {
        ...
        this.drawCircle(lowerAxis, 2, this.COLOR_BLACK, 1, true);
        p2.vec2.add(upperAxis, upperAxis, anchorA);
        this.drawCircle(upperAxis, 2, this.COLOR_BLACK, 1, true);
    }
    private drawLockJoint(joint: p2.LockConstraint): void {
        ...
        this.drawCircle(offset, 3, this.COLOR_BLACK);

        var angleIndicator:number[] = new Array();
        joint.bodyA.toWorldFrame(angleIndicator, [20, 0]);
        this.drawSegment(angleIndicator, joint.bodyA.position, this.COLOR_BLACK);
        joint.bodyB.toWorldFrame(angleIndicator, [20, 0]);
    ...
    private drawLinearSpring(spring: p2.LinearSpring): void {
        ...
        this.drawVecAt(segment, pA, this.COLOR_BLACK);
        this.drawCircle(pA, 3, this.COLOR_RED);
        this.drawCircle(anchorB, 3, this.COLOR_RED);
    }
    private drawRotationalSpring(spring: p2.RotationalSpring): void {
        ...
        spring.bodyA.toWorldFrame(pA1, [20, 0]);
        spring.bodyB.toWorldFrame(pB1, [10 * (spring.bodyB.angle - spring.bodyA.angle - spring.restAngle + 2), 0]);
        ...
        spring.bodyB.toWorldFrame(pB1, [20, 0]);
        this.drawCircle(pA1, 2, this.COLOR_RED);
        this.drawCircle(pB1, 2, this.COLOR_RED);
    }
    ...
    public drawConvexShape(shape: p2.Convex, b: p2.Body, color: number,fillColor:boolean): void {
    ...
    private drawParticle(shape: p2.Particle, b: p2.Body, color: number): void {
        this.drawCircle(b.position, 1, color, 0.5);
        this.drawCircle(b.position, 5, color, 1,false);
    }
    ...
    private drawPlaneShape(shape: p2.Plane, b: p2.Body,color:number,fillColor:boolean): void {
        ...
        worldVertices.push(worldPoint);
        worldPoint = new Array();
        b.toWorldFrame(worldPoint, [1000, -1000]);
        worldPoint = this.transformVec(worldPoint, shape.position, shape.angle);
        worldVertices.push(worldPoint);
        worldPoint = new Array();
        b.toWorldFrame(worldPoint, [-1000, -1000]);
    ...
    public drawVecAt(v: number[], at: number[], color: number, markStart: boolean = false): void {
        ...
        if (markStart) this.drawCircle(pa, 3, color);
    ...
    public drawSegment(start: number[], end: number[], color: number): void {
        ...
        this.sprite.graphics.moveTo(start[0], start[1]);
        this.sprite.graphics.lineTo(end[0], end[1]);
    }
    ...
    public drawCircle(pos: number[], radius: number, color: number, alpha: number= 1,fillColor?:boolean): void {
        ...
        this.sprite.graphics.drawCircle(pos[0], pos[1], radius);
    ...
    public drawConvex(vertices: number[][], color: number, alpha: number= 1,fillColor:boolean=true): void {
        ...
        this.sprite.graphics.moveTo(worldPoint[0], worldPoint[1]);
        for (var i: number = 1; i <= l; i++) {
            worldPoint = vertices[i % l];
            this.sprite.graphics.lineTo(worldPoint[0], worldPoint[1]);
        }
        this.sprite.graphics.endFill();
    }
