...
(function() {
  ...
  var excelFormulaUtilities = (window.excelFormulaUtilities =
    window.excelFormulaUtilities || {});
  var core = (window.excelFormulaUtilities.core = {});
  window.excelFormulaUtilities.string =
    window.excelFormulaUtilities.string || {};
   ...
   * @param {String} inStr
   **/
  var formatStr = (window.excelFormulaUtilities.string.formatStr = function(
    inStr
  ) {
 ...
 */
(function(root) {
  var excelFormulaUtilities = (root.excelFormulaUtilities =
      root.excelFormulaUtilities || {}),
  ...
  root.excelFormulaUtilities.isEu =
    typeof root.excelFormulaUtilities.isEu === "boolean"
      ? root.excelFormulaUtilities.isEu
      : false;
  ...
  function getTokens(formula) {
    ...
    while (tokens.moveNext()) {
      ...
      if (token.type.toString() === TOK_TYPE_WSPACE) {
        ...
        doAddToken =
          doAddToken &&
          ((tokens.next().type.toString() === TOK_TYPE_FUNCTION &&
            tokens.next().subtype.toString() === TOK_SUBTYPE_START) ||
            (tokens.next().type.toString() === TOK_TYPE_SUBEXPR &&
              tokens.next().subtype.toString() === TOK_SUBTYPE_START) ||
    ...
    while (tokens2.moveNext()) {
      ...
      if (
        token.type.toString() === TOK_TYPE_OP_IN &&
        token.value.toString() === "-"
      ) {
        if (tokens2.BOF()) {
      ...
      if (
        token.type.toString() === TOK_TYPE_OPERAND &&
        token.subtype.length === 0
      ) {
  ...
  var parseFormula = (excelFormulaUtilities.parseFormula = function(
    inputID,
    outputID
  ) {
    var indentCount = 0;
    ...
    while (tokens.moveNext()) {
      ...
      tokensHtml +=
        "<td class='token'>" +
        (token.subtype.length === 0 ? "&nbsp;" : token.subtype.toString()) +
        "</td>";
      tokensHtml +=
        "<td class='token'>" +
   ...
   * @returns {string}
   */
  var formatFormula = (excelFormulaUtilities.formatFormula = function(
    formula,
    options
  ) {
    ...
    var testAutoBreak = function(nextToken) {
      ...
      for (; i < autoBreakArray.length; i += 1) {
        if (
          ...
          (types[autoBreakArray[i]] === nextToken.type.toString() ||
            types[autoBreakArray[i]] === nextToken.subtype.toString())
        ) {
          return true;
        }
      }
    ...
    while (tokens.moveNext()) {
      ...
      outputFormula += applyTokenTemplate(
        token,
        options,
        indt,
        lineBreak,
   ...
   * @param {object} options optional param
   */
  var formatFormulaHTML = (excelFormulaUtilities.formatFormulaHTML = function(
    formula,
    options
  ) {
   ...
   * @returns {string}
   */
  var formula2CSharp = (excelFormulaUtilities.formula2CSharp = function(
    formula,
    options
  ) {
   ...
   * @returns {string}
   */
  var formula2JavaScript = (excelFormulaUtilities.formula2JavaScript = function(
    formula,
    options
  ) {
   ...
   * @returns {string}
   */
  var formula2Python = (excelFormulaUtilities.formula2Python = function(
    formula,
    options
  ) {
    //Custom callback to format as c#
