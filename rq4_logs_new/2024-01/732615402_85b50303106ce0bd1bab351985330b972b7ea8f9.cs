...
namespace AzureFunctionWeb.Controllers
{
    public class GroceryController : Controller
    {
        ...
        string GroceryAPIUrl = "http://localhost:7162/api/GroceryList";
        ...
        public async Task<ActionResult> Edit(GroceryItem obj)
        {
            try
            {
                ...
                using (var content = new StringContent(jsonContent, System.Text.Encoding.UTF8, "application/json"))
                {
                    HttpResponseMessage response = await client.PutAsync(GroceryAPIUrl + "/Update/" + obj.Id, content);
                    string returnValue = response.Content.ReadAsStringAsync().Result;
                }
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<ActionResult> DeletePOST(string id)
        {
            try
            {
                HttpResponseMessage response = await client.DeleteAsync(GroceryAPIUrl + "/Remove/" + id);
                string returnValue = response.Content.ReadAsStringAsync().Result;
                return RedirectToAction(nameof(Index));
            }
            catch
