...
def multiextractRoot(splited):
    ...
    subprocess.call("tar -xzf %s MusicOutDir/SpecialPxlio.pxlio"%(splited[1]),shell=True)
...
def megeRootFiles(options):
    ...
    for dataSample in dataSamples:
        command= "hadd -f9 "+outputFolder+"/"+"Data_%s.root "%(dataSample)+outputFolder+"/"+"Data_*_%s.root"%(dataSample)
        print command
