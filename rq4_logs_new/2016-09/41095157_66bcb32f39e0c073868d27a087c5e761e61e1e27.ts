...
describe('getImplicitRole', () => {
  ...
  describe('input tag', () => {
    ...
    it('when type attribute value is  email, tel, text or url and has list attribute, the implicit role is combobox',
      () => {
        chai.expect(getImplicitRole(getJsxElementFromCode('<input type="search" list />', 'input')))
          .to.equal('combobox');
        chai.expect(getImplicitRole(getJsxElementFromCode('<input type="email" list />', 'input')))
          .to.equal('combobox');
  ...
  describe('menuitem tag', () => {
    it('when type attribute value is command, the implicit role is menuitem', () => {
      chai.expect(getImplicitRole(getJsxElementFromCode('<menuitem type="command" />', 'menuitem')))
        .to.equal('menuitem');
    });

    it('when type attribute value is checkbox, the implicit role is menuitemcheckbox', () => {
      chai.expect(getImplicitRole(getJsxElementFromCode('<menuitem type="checkbox" />', 'menuitem')))
        .to.equal('menuitemcheckbox');
    });

    it('when type attribute value is radio, the implicit role is menuitemradio', () => {
      chai.expect(getImplicitRole(getJsxElementFromCode('<menuitem type="radio" />', 'menuitem')))
