...
"""
COCO_CLASSES = ('english', 'not english', 'na' )

cats_id = ['english', 'not english', 'na']

id_to_idx = dict(zip(cats_id, range(len(cats_id))))
idx_to_id = dict(zip(range(len(cats_id)), cats_id))

class CocoTextDetection(data.Dataset):
    ...
    def __init__(self, root, image_sets, transform=None, target_transform=True,
        ...
        anno_ID = list(dataset['anns'].keys())
        for i in range(len(anno_ID)):
            anno = dataset['anns'][anno_ID[i]]
            temp = dict()
            temp['id']=anno['id']
            temp['image_id']=anno['image_id']
            temp['category_id']=anno['language']
            temp['bbox']=anno['bbox']

            self.annos[anno['image_id']].append(temp)
        
        img_ID = list(dataset['imgs'].keys())
        for i in range(len(img_ID)):
            img = dataset['imgs'][img_ID[i]]
            temp = dict()
            temp['id']=img['id']
