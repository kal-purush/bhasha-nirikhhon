...
L.Control.Velocity = L.Control.extend({
  ...
  _onMouseMove: function _onMouseMove(e) {
    ...
    if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
      ...
      htmlOut = "<strong> ".concat(this.options.velocityType, " ").concat(this.options.directionString, ": </strong> ").concat(deg.toFixed(2), "\xB0").concat(cardinal, "<b>,</b> <strong> ").concat(this.options.velocityType, " ").concat(this.options.speedString, ": </strong> ").concat(self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit).toFixed(2), " ").concat(this.options.speedUnit);
    } else {
...
var Windy = function Windy(params) {
  ...
  var MAX_VELOCITY_INTENSITY = params.maxVelocity || 30; // velocity at which particle intensity is maximum (m/s)
  ...
  var FRAME_RATE = params.frameRate || 30;
  ...
  var animate = function animate(bounds, field) {
    ...
    function draw() {
      ...
      // 设置一个基础的透明度值
      g.globalAlpha = OPACITY === 0 ? 0 : OPACITY * 0.97;
    
      // 绘制粒子轨迹
      buckets.forEach(function (bucket, i) {
        if (bucket.length > 0) {
          g.beginPath();
          g.strokeStyle = colorStyles[i];  // 使用颜色样式
          bucket.forEach(function (particle) {
            // 当透明度小于 0.2 时，设置透明度为 0
            if (g.globalAlpha < 0.2) {
              g.globalAlpha = 0;  // 将透明度设置为 0
            }
