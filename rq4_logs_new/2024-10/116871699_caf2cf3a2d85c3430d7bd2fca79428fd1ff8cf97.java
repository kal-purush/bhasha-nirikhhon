...
public class TDigestAccuracyProfile implements JobProfile {
  private Job job;
  ...
  void runTrial(final int trial, final int streamLength, final int compression, final double[] ranks, final double[][] rankErrors) {
    TDigest td = TDigest.createDigest(compression);
    ...
    for (int i = 0; i < ranks.length; i++) {
      ...
      rankErrors[i][trial] = Math.abs(trueRank - td.cdf(quantile));
    }
  }
