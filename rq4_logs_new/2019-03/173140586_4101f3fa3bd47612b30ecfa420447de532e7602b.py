...
class Objective_Weight(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("P_out", units = "kW", desc = "Output power of motor in W")
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["wt"] = rho * (pi / 4) * 60 * P_out * 1000 / (pi**2 * S_stress * P_factor * var_speed) + K_gearbox * HP_out**.76 * var_speed**.13 / R_RPM**.89 
    ...
    def compute_partials(self, inputs, J):
        ...
        J["wt", "var_speed"] = -rho * (pi / 4) * 60 * P_out * 1000 / (pi**2 * S_stress * P_factor * var_speed**2) + K_gearbox * HP_out**.76 * .13 / (R_RPM**.89 * var_speed**.87)
        J["wt", "Motor_Density"] = pi / 4 * 60 * P_out * 1000 / (pi**2 * S_stress * P_factor * var_speed)
        J["wt", "P_out"] = rho * (pi / 4) * 60 / (pi**2 * S_stress * P_factor * var_speed)
        J["wt", "HP_out"] = .76 * K_gearbox * HP_out**(-.24) * var_speed**.13 / (R_RPM**.89)
        J["wt", "S_stress"] = -rho * (pi / 4) * 60 * P_out * 1000 / (pi**2 * S_stress**2 * P_factor * var_speed)
        J["wt", "P_factor"] = -rho * (pi / 4) * 60 * P_out * 1000 / (pi**2 * S_stress * P_factor**2 * var_speed)
