...
class PrimaryCaps(nn.Module):
    ...
    def squash(self, input_tensor):
        squared_norm = (input_tensor ** 2 + 1e-7).sum(-1, keepdim=True)
...
if __name__ == '__main__':
    config = CapsConfig('mnist')
    model = CapsNet(config).cuda()
    sample = torch.randn([32, 3, 32, 32]).cuda()
    output, reconstructions, masked = model(sample)
    print(output.shape)
