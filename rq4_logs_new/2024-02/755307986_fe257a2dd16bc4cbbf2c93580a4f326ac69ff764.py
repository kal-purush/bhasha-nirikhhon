...
def main():
    ...
    if not args.finetune_model:
        ...
        #from model_zoo.fast_models import PreActResNet18
        if args.wide:
            model = Wide_ResNet(depth=28).cuda()
        else:
    ...
    for epoch in range(0, args.epochs):  # loop over the dataset multiple times
        ...
        with tqdm(train_loader, unit="batch") as tepoch:
            ...
            for i, (x_loader, y_loader) in enumerate(tepoch):
                ...
                x_tr_inf, _, _, loss_best_inf, _ = apgd_train(model, x, y, norm=args.l_norms[0],
                            eps=args.l_eps[0], n_iter=args.l_iters[0], is_train=True)               

                x_tr_1, _, _, loss_best_1, _ = apgd_train(model, x, y, norm=args.l_norms[1],
                            eps=args.l_eps[1], n_iter=args.l_iters[1], is_train=True)
                
                x_tr_2, _, _, loss_best_2, _ = apgd_train(model, x, y, norm=args.l_norms[2],
                            eps=args.l_eps[2], n_iter=args.l_iters[2], is_train=True)
                
                
                y_tr = y.long().clone()

                tensor_list = [loss_best_inf, loss_best_1, loss_best_2]
                delta_list = [x_tr_inf.view(len(y),1,-1), x_tr_1.view(len(y),1,-1), x_tr_2.view(len(y),1,-1)]
                ...
                x_tr_best = x_tr_best.view(len(y), x_tr_1.shape[1], x_tr_1.shape[2], x_tr_1.shape[3])
                ...
                if args.loss in ['ce']:
                    ...
                    acc_ep_best = (outputs_best.max(dim=-1)[1] == y_tr).cpu().float().sum().item()
                    running_acc_ep_inf += acc_ep_best
                    loss = criterion(outputs_best, y_tr)
                ...
                tepoch.set_postfix({'loss': running_loss / (i+1), 'acc': running_acc_ep_inf / (i + 1) / args.batch_size})
            

        model.eval()
        
        # training stats
        stats['loss_train_dets']['clean'][epoch] = running_loss / len(train_loader) #running_loss / len(train_loader)
