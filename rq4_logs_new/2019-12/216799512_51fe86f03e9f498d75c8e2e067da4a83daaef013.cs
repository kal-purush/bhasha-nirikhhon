...
namespace Shadowsocks.Model
{
    ...
    public class DnsClient : ViewModelBase
    {
        ...
        {
            return str != null && IPAddress.TryParse(str, out var ip) && ip.ToString() == str;
        }
        ...
        private static async Task<IPAddress> QueryBaseTls(DnsOverTlsClient client, DomainName domain, DnsQueryOptions options, bool ipv6First, CancellationToken ct)
        {
            if (ipv6First)
            {
        ...
        public static async Task<IPAddress> QueryIpAddressDefault(string host, bool ipv6First, CancellationToken ct)
        {
            ...
            return await QueryBase(ARSoft.Tools.Net.Dns.DnsClient.Default, domain, options, ipv6First, ct);
        }

        public async Task<IPAddress> QueryIpAddress(string host, CancellationToken ct)
        {
            var domain = DomainName.Parse(host);
            var options = new DnsQueryOptions
            {
            ...
            switch (DnsType)
            {
                case DnsType.Default:
                {
                    ...
                    return await QueryBase(dnsClient, domain, options, Ipv6First, ct);
                }
                case DnsType.DnsOverTls:
                {
                    if (_ip == null)
