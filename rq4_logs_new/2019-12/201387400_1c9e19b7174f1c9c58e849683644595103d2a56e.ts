...
import { APIService, APIServiceClient, APIHeaders, ApiHttpProxy } from './api-service';
...
const instance = (client?: APIServiceClient, headers?: APIHeaders) => {
    const type = TYPE || 'hello';
    const endpoint = ENDPOINT || 'https://hg9errxv25.execute-api.ap-northeast-2.amazonaws.com/prod';
    const service = new APIService(type, endpoint, headers, client);
    return { service };
};
