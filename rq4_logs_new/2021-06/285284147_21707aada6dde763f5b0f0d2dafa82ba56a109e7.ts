    ...
    context('with staked and registered relay', function () {
      ...
      context('with funded verifier', function () {
        ...
        it('relayCall atomicity cannot be broken using user-entered gas attacks', async function () {
          this.timeout(0)
          let gasSent: number = 42300 // less gas wont even pass the node's transaction cost validation
          ...
          while (!succeed) {
            ...
            const workerTokenBalanceBefore = await token.balanceOf(relayWorker)
            const workerRBTCBalanceBefore = toBN(await web3.eth.getBalance(relayWorker))
            const swTokenBalanceBefore = await token.balanceOf(forwarder)
            const swRBTCBalanceBefore = toBN(await web3.eth.getBalance(forwarder))
            const ownerTokenBalanceBefore = await token.balanceOf(gaslessAccount.address)
            const ownerRBTCBalanceBefore = toBN(await web3.eth.getBalance(gaslessAccount.address))
            let transaction: Truffle.TransactionResponse

            try {
              transaction = await relayHubInstance.relayCall(relayRequest, signatureWithPermissiveVerifier, {
                from: relayWorker,
                gasPrice,
                gas: gasSent
              })
              ...
              assert.isTrue(nonceBefore.eq(nonceAfter), 'Smart wallet nonce must be unchanged')

              const workerTokenBalanceAfter = await token.balanceOf(relayWorker)
              assert.isTrue(workerTokenBalanceBefore.eq(workerTokenBalanceAfter), 'Worker token balance must be unchanged')

              const swTokenBalanceAfter = await token.balanceOf(forwarder)
              assert.isTrue(swTokenBalanceBefore.eq(swTokenBalanceAfter), 'Smart wallet token balance must be unchanged')

              const workerRBTCBalanceAfter = toBN(await web3.eth.getBalance(relayWorker))

              if (JSON.stringify(error).includes('basic cost is above the gas limit')) {
                assert.isTrue(workerRBTCBalanceBefore.eq(workerRBTCBalanceAfter), `RBTC balance of the worker must be the same, gasSent is ${gasSent}, balance before is ${workerRBTCBalanceBefore.toString()}, balance after is ${workerRBTCBalanceAfter.toString()}`)
              } else {
                assert.isTrue(workerRBTCBalanceBefore.gt(workerRBTCBalanceAfter), `RBTC balance of the worker must have decreased, gasSent is ${gasSent}, balance before is ${workerRBTCBalanceBefore.toString()}, balance after is ${workerRBTCBalanceAfter.toString()}`)
              }

              const swRBTCBalanceAfter = toBN(await web3.eth.getBalance(forwarder))
              assert.isTrue(swRBTCBalanceBefore.eq(swRBTCBalanceAfter), 'Smart Wallet RBTC balance, if any, must be unchanged')

              const ownerTokenBalanceAfter = await token.balanceOf(gaslessAccount.address)
              assert.isTrue(ownerTokenBalanceBefore.eq(ownerTokenBalanceAfter), 'Smart wallet owner token balance, if any, must be unchanged')

              const ownerRBTCBalanceAfter = toBN(await web3.eth.getBalance(gaslessAccount.address))
              assert.isTrue(ownerRBTCBalanceBefore.eq(ownerRBTCBalanceAfter), 'Smart Wallet owner RBTC balance, if any, must be unchanged')
            ...
            const receipt = await web3.eth.getTransactionReceipt(transaction.tx)
            ...
            const workerTokenBalanceAfter = await token.balanceOf(relayWorker)
            assert.isTrue(workerTokenBalanceBefore.add(toBN(1)).eq(workerTokenBalanceAfter), 'Worker token balance did not increase')

            const swTokenBalanceAfter = await token.balanceOf(forwarder)
            assert.isTrue(swTokenBalanceBefore.eq(swTokenBalanceAfter.add(toBN(1))), 'Smart wallet token balance did not decrease')
