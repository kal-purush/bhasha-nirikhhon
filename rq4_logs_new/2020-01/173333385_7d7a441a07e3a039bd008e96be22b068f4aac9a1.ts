...
import { STANDARD_ERRORS, WALLET_ADDRESS } from './constants'
...
export const getClaymoreEthashNanopoolDefinition = (machine: Machine): PluginDefinition => {
  let def = {
    ...
    args: `${getPool('eth-us-west1')} ${getPool('eth-us-east1')} ${getPool('eth-eu1')} ${getPool('eth-eu2')} -ewal ${WALLET_ADDRESS} -epsw x -eworker ${machine.id} -allpools 1 -allcoins 0`,
