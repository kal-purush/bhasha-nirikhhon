...
public class TabuSearchEdges extends TSPLocalSearchAlgorithm<int[]> {
  ...
  /** the tabu list */
  private ArrayList<_PairOfNode> inComingTabuList;
  private ArrayList<_PairOfNode> outComingTabuList;
  ...
  public void localSearch(final Individual<int[]> srcdst,
    ...
    final int n;
    ArrayList<_PairOfNode> inComingEdges, outComingEdges;
    ...
    while (foundImprov && (!(f.shouldTerminate()))) {
        ...
        for (b = a + 1; b < (n - 1); b++) {
          for (final PermutationUpdateOperator o : this.m_ops) {
            ...
            if (d < delta) {
            	...
            	if(!inComingTabuList.isEmpty()){
            		for (int i = 0 ; i < inComingEdges.size(); i++) {
            			if(outComingTabuList.contains(inComingEdges.get(i))
							||inComingTabuList.contains(outComingEdges.get(i)))
            				isTabued = true;
            		}
            	}
            	if(!isTabued){
      ...
      if (best != null) {
        ...
        inComingTabuList.addAll(inComingEdges);
        outComingTabuList.addAll(outComingEdges);
        for(int i = inComingTabuList.size()-this.tabuListLength-1 ; i >= 0 ;i--){
        	inComingTabuList.remove(i);
        	outComingTabuList.remove(i);
        }
        value += delta;
  ...
  public final TabuSearchEdges clone() {
    ...
    res.inComingTabuList = null;
    res.outComingTabuList = null;
    return res;
  }
  ...
  public void beginRun(final ObjectiveFunction f) {
    ...
    if ((this.absolute + (int) (this.rate * Math.sqrt(f.n()))) < (f.n() * (f
        .n() - 1))) {
    ...
    this.inComingTabuList = new ArrayList<_PairOfNode>(this.tabuListLength);
    this.outComingTabuList = new ArrayList<_PairOfNode>(this.tabuListLength);
  }

  /** {@inheritDoc} */
  @Override
  public void endRun(final ObjectiveFunction f) {
    this.inComingTabuList = null;
    this.outComingTabuList = null;
    try {
