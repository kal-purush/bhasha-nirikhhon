    ...
    Trumbowyg.prototype = {
        ...
        openModalInsert: function (title, fields, cmd) {
            ...
            $.each(fields, function (fieldName, field) {
                ...
                // added non value attributes to the input tag (multiple, required)
                var fieldNonValueAttrs = [];
                for (var key in field) {
                  if (field[key] === true) {
                    fieldNonValueAttrs.push(key);
                  }
                }

                html += '<label><input type="' + (field.type || 'text') + '" name="' + n + '" value="' + (field.value || '') + '" ' + fieldNonValueAttrs.join(' ') + '><span class="' + prefix + 'input-infos"><span>' +
