...
public class BikeOwnershipReader extends AbstractCsvReader {

    private static final Logger logger = Logger.getLogger(BikeOwnershipReader.class);
    ...
    public BikeOwnershipReader(DataSet dataSet) {
        super(dataSet);
        this.dataSet = dataSet;
        path = AbitResources.instance.getString("bike.ownership");
        super.read(Path.of(path), ",");
    }
    ...
    public void assignBicycleOwnership(Person person) {

        //for (Household hh : dataSet.getHouseholds().values()) {

        int economicStatus = person.getHousehold().getEconomicStatus().ordinal();
        BBSRType areaTypeSG = dataSet.getZones().get(person.getHousehold().getLocation().getZoneId()).getBBSRType();
        int cars = person.getHousehold().getNumberOfCars();
        double distanceToTransit = dataSet.getZones().get(person.getHousehold().getLocation().getZoneId()).getDistToRail_meter(); //the model says distance to transit!
...
//                    bikesPerAgeBin.put(ageBin, bikesPerAgeBin.get(ageBin) + 1);
        ...
        person.setHasBicycle(hasBicycle);
