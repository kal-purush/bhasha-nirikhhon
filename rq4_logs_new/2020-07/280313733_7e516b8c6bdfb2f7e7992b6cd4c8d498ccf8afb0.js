...
function make (target) {
  ...
  const pid = spawn('make', justDir, ['-C', justDir, target], stdin[1], stdout[1], stderr[1])
  close(stdin[0])
...
function build (config, onComplete = () => {}) {
  ...
  try {
    ...
    write(STDOUT_FILENO, ArrayBuffer.fromString(` ${ANSI_MAGENTA}time${ANSI_DEFAULT} ${getTime()}\n`))
    const target = just.args[2] || 'runtime'
    write(STDOUT_FILENO, ArrayBuffer.fromString(`${ANSI_YELLOW}building ${target}${ANSI_DEFAULT}\n`))
    const { pid, stdout, stderr } = make(target)
...
let justDir = `${just.env().JUST_HOME || just.sys.cwd()}/.just`
