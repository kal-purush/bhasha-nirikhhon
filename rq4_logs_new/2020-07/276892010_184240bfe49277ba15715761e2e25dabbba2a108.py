...
class LeastArea:
    ...
    def GetData(self, container):
        ...
        end_data = [[i for i, j in least_cameras.items()],[[item for k, item in j.items()] for i, j in least_cameras.items()],[[ self.MakeAcronym(k) for k, item in j.items()] for i, j in least_cameras.items()]]
