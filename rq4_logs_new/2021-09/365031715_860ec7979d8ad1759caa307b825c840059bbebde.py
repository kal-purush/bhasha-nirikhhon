...
class TestHealpixSubsetSlicerSetup(unittest.TestCase):
    ...
    def testSlicertype(self):
        ...
        testslicer = HealpixSubsetSlicer(nside=self.nside, hpid=self.idxs, verbose=False,
...
class TestHealpixSlicerEqual(unittest.TestCase):
    ...
    def setUp(self):
        self.cameraFootprintFile = os.path.join(get_data_dir(), 'tests', 'fov_map.npz')
        self.nside = 16
        self.testslicer = HealpixSubsetSlicer(nside=self.nside, hpid=np.arange(0, 10), verbose=False,
    ...
    def testSlicerEquivalence(self):
        ...
        testslicer2 = HealpixSubsetSlicer(nside=self.nside, hpid=np.arange(0, 10),
        ...
        testslicer2 = HealpixSubsetSlicer(nside=self.nside, hpid=np.arange(10, 20),
                                          verbose=False, lonCol='ra', latCol='dec',
                                          cameraFootprintFile=self.cameraFootprintFile)
...
class TestHealpixSlicerSlicing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.nside = 8
        self.radius = 1.8
        self.testslicer = HealpixSubsetSlicer(nside=self.nside, hpid=np.arange(0, hp.nside2npix(self.nside)),
                                              verbose=False, lonCol='ra', latCol='dec', latLonDeg=False,
    ...
    def testSubsetSlice(self):
        testslicer = HealpixSubsetSlicer(nside=self.nside, hpid=np.arange(0, 2),
                                         verbose=False, lonCol='ra', latCol='dec', latLonDeg=False,
                                         radius=self.radius, useCamera=False)
