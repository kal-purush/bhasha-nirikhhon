...
def indicator_table(data):
    ...
    table = [(gettext('Indicator Level'),
              Paragraph(level)),
              (gettext('Indicator Assesment'),
              Paragraph(gettext("%s.indicator" % data['name']))),
             (gettext('Technical Implementation'), Paragraph(gettext("%s.technical" % data['name']))),
             (gettext('Technical feedback'), Paragraph(data['msg'])),
             (gettext('Tips'), Paragraph(gettext("%s.tips" % data['name'])))]
    return table
...
def add_group_indicators(Story, data_indicators, name, estilos):
    ...
    for k in data_indicators:
        if k != 'result':
            print(k)
            Story.append(Paragraph(gettext(data_indicators[k]['name']), estilos["centerBold"]))
