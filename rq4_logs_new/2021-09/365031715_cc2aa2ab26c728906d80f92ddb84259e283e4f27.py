...
class SFErrorMetric(BaseMetric):
    ...
    def __init__(self, mag=22, timesCol='observationStartMJD', m5Col='fiveSigmaDepth',
                 allGaps=True, units='mag',
                 bins=np.logspace(0, np.log10(3650), 11), weight=np.full(10, 0.1), 
                 metricName='Structure Function Error', snr_cut=5,
                 filterCol='filter', dust=True, **kwargs):
        ...
        maps = ['DustMap']
        super(SFErrorMetric, self).__init__(col=[self.timesCol, m5Col, filterCol], 
                                            metricName=self.metricName, units=units,
                                            maps=maps, **kwargs)
    ...
    def run(self, dataSlice, slicePoint=None):
        ...
        mag_err = 2.5*np.log10(1.+1./snr[bright_enough][order])
