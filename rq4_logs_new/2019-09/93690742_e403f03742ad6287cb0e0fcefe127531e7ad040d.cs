...
namespace SJP.Schematic.Reporting.Html.Renderers
{
    internal sealed class TableOrderingRenderer : ITemplateRenderer
    {
        ...
        public async Task RenderAsync(CancellationToken cancellationToken = default)
        {
            ...
            await ExportInsertionOrder(hasCycles).ConfigureAwait(false);
            await ExportDeletionOrder(hasCycles).ConfigureAwait(false);
        }

        private async Task ExportInsertionOrder(bool hasCycles)
        {
