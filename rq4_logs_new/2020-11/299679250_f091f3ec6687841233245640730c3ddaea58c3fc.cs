...
namespace AV.Hierarchy
{
    internal class HoverPreview : VisualElement
    {
        ...
        internal SmartHierarchy hierarchy;
        internal TreeViewState state => hierarchy.state;
        internal EditorWindow window => hierarchy.window.actualWindow;
        ...
        {
            container.style.opacity = Mathf.Clamp01(showingPreviewAnim.faded * 50);
        ...
        private void SetHoverPreviewPosition()
        {
            ...
            rect.x = Mathf.Clamp(rect.x, 0, window.position.xMax - 16 - size.x);
            rect.y = Mathf.Clamp(rect.y, 24, window.position.yMax - 100 - size.y);
