...
def convolve2d(img, kernel):
    ...
    flipped_kernel=utils.flip2d(kernel, None)
    ...
    for i in range(1,len(padded_img)-2):
        for j in range(1,len(padded_img[0])-2):
            cropped_img=utils.crop(padded_img,i-1, i+2, j-1, j+2)
            element_matrix=utils.elementwise_mul(cropped_img, flipped_kernel)
            val=0
            for x in element_matrix:
                for y in x:
                    val+=y
            img_conv[i][j]=val
...
def normalize(img):
    ...
    """
    # TODO: implement this function.
    normalized_img = np.divide((img - np.amin(img)),(np.amax(img) - np.amin(img)))
    normalized_img=normalized_img* 255
    normalized_img = [list(row) for row in normalized_img]
    # raise NotImplementedError
...
def detect_edges(img, kernel, norm=True):
    ...
    if(norm==True):
        ...
        img_edge=normalize(np.asarray(conv_img))
    else:
