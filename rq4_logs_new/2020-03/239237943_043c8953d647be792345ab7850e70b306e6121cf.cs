...
namespace WorkEngineKata.Tests
{
    public class UnitTest1
    {
        [Fact]
        public void It_Should_Upload_A_Video_To_Cloud_When_Run()
        {
            // var workFlowEngine = new WorkFlowEngine();
            //
            // var run = workFlowEngine.Run();
            //
            // Assert.Equal("Uploading a video to cloud storage", run);
        }
    }
}
...
// activity in the workflow and runs it.We want our workflows to be extensible, so we can create new activities without
// impacting the existing activities. Educational tip: we should represent the concept of an activity using an interface. 
// Each activity should have a method called Execute(). The workflow engine does not care about the concrete implementation
// of activities. All it cares about is that these activities have a common interface: they provide a method called Execute(). 
...
// 4- Change the status of the video record in the database to “Processing”.Each of these steps can be represented by an activity. 
