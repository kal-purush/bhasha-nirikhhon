...
public class Utils {
	...
	public static String ORG_NEUROML_EXPORT_VERSION = "1.4.1";
	...
	public static String getHeaderComment(String format) {
		String commentString = "    This "
				+ format
				+ " file has been generated by org.neuroml.export (see https://github.com/NeuroML/org.neuroml.export)\n"
				+ "         org.neuroml.export  v"
				+ Utils.ORG_NEUROML_EXPORT_VERSION + "\n"
				+ "         org.neuroml.model   v"
				+ NeuroMLElements.ORG_NEUROML_MODEL_VERSION + "\n"
				+ "         jLEMS               v"
	...
	/*
	 * Gets the magnitude of a NeuroML 2 quantity string in SI units (e.g. -60mV
	 * -> -0.06)
	 */
	public static float getMagnitudeInSI(String nml2Quantity)
		...
		try {
			DimensionalQuantity dq = QuantityReader.parseValue(nml2Quantity,
					getLemsWithNML2CompTypes().getUnits());
			float val = (float) dq.getValue();

			return val;
		} catch (LEMSException ex) {
			throw new NeuroMLException("Problem getting magnitude in SI for: "
					+ nml2Quantity, ex);
		}
	...
	/*
	 * Gets the Dimension of a NeuroML 2 quantity string in SI units (e.g. -60mV
	 * returns Dimension with values for Voltage)
	 */
	public static Dimension getDimension(String nml2Quantity)
			throws NeuroMLException {
	...
	public static String parseCellRefStringForPopulation(String cellRef) {
		...
		String ref = cellRef.indexOf("/") >= 0 ? cellRef.split("/")[loc]
				: cellRef;
		if (ref.indexOf("[") >= 0)
	...
	/*
	 * For example, ../Pop0[0] returns 0; ../Gran/0/Granule_98 returns 0;
	 * Gran/1/Granule_98 returns 0
	 */
	public static int parseCellRefStringForCellNum(String cellRef) {
		...
		if (cellRef.indexOf("[") >= 0) {
			return Integer.parseInt(cellRef.substring(cellRef.indexOf("[") + 1,
					cellRef.indexOf("]")));
		} else {
	...
	public static Unit getSIUnitInNeuroML(Dimension dim)
			throws NeuroMLException {
		try {
			for (Unit unit : getLemsWithNML2CompTypes().getUnits()) {
				if (unit.getDimension().getName().equals(dim.getName())
						&& unit.scale == 1 && unit.power == 0
						&& unit.offset == 0)
					return unit;
			}
		} catch (LEMSException ex) {
			throw new NeuroMLException(
					"Problem finding SI unit for dimension: " + dim, ex);
		}
	...
	public static Sim readLemsNeuroMLFile(String contents) throws LEMSException {
		...
		JarResourceInclusionReader jrir = new JarResourceInclusionReader(
				contents);
	...
	public static String replaceInExpression(String expression, String oldVal,
			String newVal) {
	...
	public static LinkedHashMap<String, Standalone> convertLemsComponentToNeuroML(
			Component comp) throws LEMSException, NeuroMLException {
		XMLSerializer xmlSer = XMLSerializer.newInstance();
		String compString = xmlSer.writeObject(comp);

		NeuroMLConverter nmlc = new NeuroMLConverter();
		NeuroMLDocument nmlDocument = nmlc
				.loadNeuroML("<neuroml xmlns=\"http://www.neuroml.org/schema/neuroml2\"\n"
						+ "    xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n"
						+ "      xsi:schemaLocation=\"http://www.neuroml.org/schema/neuroml2 "
						+ NeuroMLElements.LATEST_SCHEMA_LOCATION
						+ "\">"
						+ compString + "</neuroml>");
		LinkedHashMap<String, Standalone> els = NeuroMLConverter
				.getAllStandaloneElements(nmlDocument);
		return els;
	}

	public static Component convertNeuroMLToComponent(Standalone nmlElement)
			throws NeuroMLException, LEMSException {
	...
	public static Cell getCellFromComponent(Component comp)
			throws LEMSException, NeuroMLException {
		LinkedHashMap<String, Standalone> els = Utils
				.convertLemsComponentToNeuroML(comp);
		Cell cell = (Cell) els.values().iterator().next();
		return cell;
	}

	public static Sim convertNeuroMLToSim(Standalone nmlElement)
			throws NeuroMLException, LEMSException {
				...
				for (int k = 0; k < j; k++) {
					if (ascending) {
						if (list.get(j).toString()
								.compareToIgnoreCase(list.get(k).toString()) < 0) {
	...
	public static void runLemsFile(File f) throws LEMSException,
			ModelFeatureSupportException, NeuroMLException {
		loadLemsFile(f, true);
	}
	...
	public static void loadLemsFile(File f, boolean run) throws LEMSException,
		...
		if (run) {
			...
			sli.checkAllFeaturesSupported(
					SupportLevelInfo.LEMS_NATIVE_EXECUTION, sim.getLems());
			sim.run();
	...
	/**
	 * @return i686 for most, x86_64 if "64" present in system properties
	 *         os.arch, e.g. amd64. Will need updating as Neuron tested on more
	 *         platforms...
	 * 
	 */
	public static String getArchSpecificDir() {
		if (!isMacBasedPlatform()
				&& (System.getProperty("os.arch").equals(ARCH_64BIT) || System
						.getProperty("os.arch").indexOf("64") >= 0)) {
			return DIR_64BIT;
		} else if (isMacBasedPlatform()
				&& System.getProperty("os.arch").indexOf(ARCH_POWERPC) >= 0) {
			return DIR_POWERPC;
		} else if (isMacBasedPlatform()
				&& System.getProperty("os.arch").indexOf(ARCH_I386) >= 0) {
			return DIR_I686;
		} else {
			return DIR_I686;
		}
