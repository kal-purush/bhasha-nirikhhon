...
class ContMarkov(SequenceEvo):
	...
	def simulation(self, start="random"):
		...
		while time_passed <= self.time: #Does sampling till time elapsed is greater than total time specified
			...
			list = filter(lambda x:x != start, self.sequence) #Get list of variables it can change to
			prob = filter(lambda x:x != -1, self.values[start]) #Get list of probabilities for these variables
			nextval = Markov().sample(list, prob) #Samples new distribution
	...
	def endFreqs(self, start, trials):
		...
		for x in range(trials):
			...
			endvals.append(output1[len(output1)-1])
		freqlist = {}
		for val in endvals:
			if val in freqlist:
				freqlist[val]+=1
...
model = ContMarkov(["A", "C", "G", "T"], [[-1.916, 0.541, 0.787, 0.588], [0.148, -1.069, 0.415, 0.506], [0.286, 0.170, -0.591, 0.135], [0.525, 0.236,0.594, -1.355]], time = 10)
...
#model.endFreqs(start = "A", trials=1000) #Use this to calculate frequency of an allele after certain generations