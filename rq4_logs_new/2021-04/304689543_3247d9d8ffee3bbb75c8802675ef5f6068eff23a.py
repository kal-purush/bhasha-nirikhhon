...
@contextmanager
def mock_redact_server(expected_path: Optional[str] = None, expected_job_args: Optional[JobArguments] = None):
    """
    ...
    def get_all_routes(request: Request):
        return _mock_redact_request_handler(request=request,
                                            expected_path=expected_path,
                                            expected_job_args=expected_job_args)

    server = Process(target=uvicorn.run, args=(app,), kwargs={'port': 8787}, daemon=True)
    server.start()
...
def _mock_redact_request_handler(request: Request,
    ...
    if expected_path and actual_path != expected_path:
        return Response(status_code=500, content=f'Accessed path {actual_path} != expected path {expected_path}')

    # job_args
    actual_job_args = JobArguments.parse_obj(request.query_params)
    if expected_job_args and actual_job_args != expected_job_args:
        return Response(status_code=500, content=f'Received query args {repr(actual_job_args)} != expected query args {repr(expected_job_args)}')

    return Response(status_code=200, content=JobPostResponse(output_id=uuid.uuid4()).json())