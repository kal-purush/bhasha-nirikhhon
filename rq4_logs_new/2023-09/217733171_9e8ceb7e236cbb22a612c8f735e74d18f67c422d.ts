...
const options: Hierarchy.Options = { x: 600, y: size.width, nodeSize: [50, 600], anchor: 'left' }
const data = [
  hierarchy(root, { nodes, edges, options: { ...options, alignment: 'min' } }),
  hierarchy(root, { nodes, edges, options: { ...options, alignment: 'mid' } }),
  hierarchy(root, { nodes, edges, options: { ...options, alignment: 'max' } })
]
