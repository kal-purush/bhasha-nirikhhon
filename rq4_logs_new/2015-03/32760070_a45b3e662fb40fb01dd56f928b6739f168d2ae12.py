...
if __name__ == "__main__":
  ...
  try:
    ...
    if argc == 3 or argc == 5:
      ...
      slave_1.add_block('ro', cst.HOLDING_REGISTERS, int(sys.argv[1]), 2*int(sys.argv[2]))
      ro = range(int(sys.argv[1]), int(sys.argv[1]) + 2*int(sys.argv[2]))
    if argc == 5:
      logger.info("Initializing %s RW float registers beginning at %s" % (str(sys.argv[4]), str(sys.argv[3])))
      slave_1.add_block('rw', cst.HOLDING_REGISTERS, int(sys.argv[3]), 2*int(sys.argv[4]))
      rw = range(int(sys.argv[3]), int(sys.argv[3]) + 2*int(sys.argv[4]))
      # get initial values from the AVR
      for i in rw[0::2]:
        s = avr.get(str(i))
        if s is None:
          s = "0"
        i1,i2 = struct.unpack('>HH',struct.pack('f',float(s)))
        slave_1.set_values('rw', i, i2)
        slave_1.set_values('rw', i+1, i1)
