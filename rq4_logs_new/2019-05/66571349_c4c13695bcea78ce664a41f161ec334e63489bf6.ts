...
import {compareByPtmTypePriority, groupPtmDataByPosition, groupPtmDataByTypeAndPosition, ptmColor} from "./PtmUtils";
...
describe("PtmUtils", () => {
    ...
    describe('compareByPtmTypePriority', () => {
        it('compares ptm types by priority', () => {
            assert.equal(compareByPtmTypePriority("Phosphorylation", "Phosphorylation"), 0,
                "identical strings should return 0");
            assert.equal(compareByPtmTypePriority("Phosphorylation", "Acetylation"), -1,
                "Phosphorylation has a higher priority than Acetylation");
            assert.equal(compareByPtmTypePriority("Phosphorylation", "Methylation"), -1,
                "Phosphorylation has a higher priority than Methylation");
            assert.equal(compareByPtmTypePriority("Methylation", "Acetylation"), 1,
                "Methylation has a lower priority than Acetylation");
            assert.equal(compareByPtmTypePriority("Ubiquitination", "Sumoylation"), -1,
                "Ubiquitination has a higher priority than Sumoylation");
            assert.equal(compareByPtmTypePriority("Unknown", "Sumoylation" ), 1,
    ...
    describe('ptmColor', () => {
        it('picks the correct color for a list of PTMs', () => {
            assert.equal(ptmColor(ptmData), "#444444",
                "mixed PTM types should have a special color");
            assert.equal(ptmColor([ptmData[4]]), "#2DCF00",
                "Phosphorylation: #2DCF00");
            assert.equal(ptmColor([ptmData[0], ptmData[3]]), "#5B5BFF",
                "Acetylation: #5B5BFF");
            assert.equal(ptmColor([ptmData[1], ptmData[2],  ptmData[6]]), "#B9264F",
                "Ubiquitination: #B9264F");
            assert.equal(ptmColor([ptmData[5]]), "#EBD61D",
                "Methylation: #EBD61D");
            assert.equal(ptmColor([ptmData[7]]), "#BA21E0",
                "Glutathionylation (default): #BA21E0");
            assert.equal(ptmColor([ptmData[8]]), "#BA21E0",
