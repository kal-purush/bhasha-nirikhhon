...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        print(pred[...,4].max())
        # Apply NMS
        pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms, kpt_label=kpt_label, nkpt=nkpt)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for det_idx, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    ...
                    print(f" {{ \"id\": \"{det_idx + 1}\", \"label\": \"cars\", \"x\": \"{center_x:.4f}\", \"y\": \"{center_y:.4f}\", \"width\": \"{width:.4f}\", \"height\": \"{height:.4f}\",")

                    kpts = det[0, 6:].reshape(-1, 3)
                    keypoints_str = " \"keypoints\": ["
                    for k in range(8):
                        x, y = kpts[k][0], kpts[k][1]
                        keypoints_str += f"{{\"id\": {k}, \"x\": \"{x:.4f}\", \"y\": \"{y:.4f}\"}}"
                        if k < 7:
                            keypoints_str += ", "
                    keypoints_str += "]"
                    print(keypoints_str)
