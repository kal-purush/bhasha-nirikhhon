...
public interface DefectEndpoint {
    ...
    TaskListJson getDefects(
        ...
        @Parameter(description = "Filter to return only defects of a specific user") @QueryParam("owner") @UUID String ownerId,
        @Parameter(description = "Filter to return only defects with a specific status") @QueryParam("status") Status status);
    ...
    @APIResponse(responseCode = "401", description = "No user authentication provided via session cookie")
    @APIResponse(responseCode = "404", description = "The defect does not exist")
    ...
    @APIResponse(responseCode = "204", description = "The defect was deleted successfully")
    @APIResponse(responseCode = "401", description = "No user authentication provided via session cookie")
    void deleteDefect(
        @Parameter(description = "ID of the project", required = true) @PathParam("projectId") @NotNull @UUID String projectId,
        @Parameter(description = "ID of the defect", required = true) @PathParam("defectId") @NotNull @UUID String defectId);
