...
class DOTA:
    ...
    def saveAnns(self, objects, imgId, range, save_path):
        """
        :param catNms: category names
        :param objects: objects to save
        :param imgId: img to save
        :param range: display range in the img
        :return:
        """
        img = self.loadImgs(imgId)[0]
...
if __name__ == '__main__':
    examplesplit = DOTA('dataset/dataset_demo_rate1.0_split1024_gap200')
    imgids = examplesplit.getImgIds(catNms=['plane'])
    img = examplesplit.loadImgs(imgids)
    for imgid in imgids:
        anns = examplesplit.loadAnns(imgId=imgid)
        examplesplit.saveAnns(anns, imgid, 2, 'dataset/dataset_demo_rate1.0_split1024_gap200/vis.png')