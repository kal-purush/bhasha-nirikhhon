...
const emptyAuth = {
  ...
  session() { return Option.none(); },
  actor() { return Option.none(); },
  isAuthenticated() { return false; }
};

// actor is required. session may or may not exist.
const actorAuth = (actor, session, Auth) => ({
  ...
  session() { return Option.of(session); },
  actor() { return Option.of(actor); },
  isAuthenticated() { return true; }
});

// the actual interface, which determines what sort of response to compose.
const by = (x) => ({ Auth }) => (
  (x instanceof Session) ? actorAuth(session.actor, session, Auth) :
  (x instanceof Actor) ? actorAuth(actor, undefined, Auth) :
  emptyAuth
);


module.exports = { can, canGrant, verbsOn, local, by };
