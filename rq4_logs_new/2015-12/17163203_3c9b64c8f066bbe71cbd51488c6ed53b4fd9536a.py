...
class NewsStateFilter(django_filters.FilterSet):
  ...
  def __init__(self, *args, **kwargs):
    ...
    choices = None
    choices = list(State.objects.filter(id__in=News.objects.filter(news_type__slug=news_type_slug).values('state_id').distinct()).order_by('name').values_list('id', 'name'))
    choices.insert(0, ('', '---------'),)
