...
class RizzoPlugin(idaapi.plugin_t):
    ...
    # Actions
    action_rizzo_loadfile = 'rizzo:loadfile'
    action_rizzo_producefile = 'rizzo:producefile'
    ...
    def init(self):
        if idaapi.IDA_SDK_VERSION >= 700:
            idaapi.register_action(idaapi.action_desc_t(
                ...
                self.rizzo_loadHandler(),
                None,
                'Rizzo signature file...'))
            idaapi.attach_action_to_menu("File/Load file/",
                self.action_rizzo_loadfile, idaapi.SETMENU_APP)

            idaapi.register_action(idaapi.action_desc_t(
                self.action_rizzo_producefile,
                'Rizzo signature file...',
                self.rizzo_produceHandler(),
                None,
                'Rizzo signature file...'))
            idaapi.attach_action_to_menu("File/Produce file/",
                self.action_rizzo_producefile, idaapi.SETMENU_APP)
        else:
    ...
    class rizzo_loadHandler(idaapi.action_handler_t):
        def __init__(self):
    ...
    class rizzo_produceHandler(idaapi.action_handler_t):
        def __init__(self):
            idaapi.action_handler_t.__init__(self)
            
        def activate(self, ctx):
            rizzo_produce(self, 0)
