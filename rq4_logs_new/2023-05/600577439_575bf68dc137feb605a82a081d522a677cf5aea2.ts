...
async function chronicle_source(
  request: NextApiRequest,
  response: NextApiResponse,
  infos: BroadcastCtx
) {
  const { editor, myLocalId } = infos
  const { source, id: chronicleId } = JSON.parse(request.body)

  if (!editor && !source && chronicleId)
...
const helper = (request: NextApiRequest, response: NextApiResponse) =>
  broadcastMiddleWare(request, response, chronicle_source)
