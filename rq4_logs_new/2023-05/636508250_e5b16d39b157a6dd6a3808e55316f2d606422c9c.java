...
public class PathFinder {
    ...
    public void instantiateNodes(){
        ...
        while(col < gp.maxWorldCol && row < gp.maxWorldRow){
            node[col][row] = new Node(col, row);//create node for each tile of the map

            col++;
            if(col == gp.maxWorldCol){//do we need another if to set the second map ???
                col = 0;
                row++;
            }
        }
    ...
    public boolean search(){
        while(!goalReached && step < 500){
            ...
            for( int i = 0; i < openList.size(); i++){
                ...
                //If F cost is equa, check the G cost
