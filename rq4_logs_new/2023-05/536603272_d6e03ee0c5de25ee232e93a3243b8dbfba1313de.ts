...
async function generateAuthorizationJwt({ origin }: URL) {
    // Create the payload
...
export async function createPushPayload(
    ...
    return new Request(endpoint, {
        ...
        body: rawBody,
        headers: {
            'Authorization': await generateAuthorizationJwt(new URL(endpoint)),
            'Content-Encoding': 'aesgcm',
            'Content-Length': rawBody.byteLength.toString(),
            'Content-Type': 'application/octet-stream',
            'Crypto-Key': `dh=${b64encode(rawLocalPublicKey)}; p256ecdsa=${ENCODED_VAPID_PUB_KEY}`,
