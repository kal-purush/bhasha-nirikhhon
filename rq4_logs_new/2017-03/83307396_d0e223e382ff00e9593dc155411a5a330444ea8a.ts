...
.on('message', m => {
  try {
    ...
    var fs=require('fs');
    //fs.writeFileSync('./dialog.json',JSON.stringify({a:1,b:2}));
    var MyJsonObj=JSON.parse(fs.readFileSync('./dialog.json'));
    for(var key in MyJsonObj){
        //console.log(key+':'+MyJsonObj[key] +'\n')
	if((m.content().toLowerCase().indexOf(key.toLowerCase()) >=0)  && !m.self() && m.room() ){
	    m.say(MyJsonObj[key])
	}
    }
