...
describe(commands.YAMMER_REPORT_DEVICEUSAGEUSERDETAIL, () => {
  let vorpal: Vorpal;
  let log: string[];
  let cmdInstance: any;
  ...
  it('has correct name', () => {
    assert.equal(command.name.startsWith(commands.YAMMER_REPORT_DEVICEUSAGEUSERDETAIL), true);
  });
  ...
  it('gets the report for the last week', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, period: 'D7' } }, () => {
      try {
        assert.equal(requestStub.lastCall.args[0].url, "https://graph.microsoft.com/v1.0/reports/getYammerDeviceUsageUserDetail(period='D7')");
  ...
  it('has help referring to the right command', () => {
    ...
    cmd.help({}, () => { });
    assert(find.calledWith(commands.YAMMER_REPORT_DEVICEUSAGEUSERDETAIL));
  });
});