...
def scan():
    ...
    _marker = '~~~~~~~~~~~~~~'
    p = subprocess.run(["heimdall","-scan", _pwd, _file_list], capture_output=True)
    _byte_output = p.stdout
    _output = _byte_output.decode()
    _start_index = _output.find(_marker)
    _final_output = _output[_start_index:].strip()
    print('\n')
    _exit_code = 0
...
def revert_commit(root_dir):
    p = subprocess.run(["cd", root_dir], capture_output=True)
    p = subprocess.run(["git", "reset", "*"], capture_output=True)
