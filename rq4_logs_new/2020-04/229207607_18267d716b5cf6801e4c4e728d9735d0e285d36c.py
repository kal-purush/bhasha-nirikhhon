...
def sctp_parser(ip):
    ...
    sctp.psm.hs2 = (INIT_SENT >> INIT_ACK_SENT) + Pred(sctp.header_contain(sctp_init_ack_hdr))
    sctp.psm.hs3 = (INIT_ACK_SENT >> COOKIE_ECHO_SENT) + Pred(sctp.header_contain(sctp_cookie_echo_hdr))
    sctp.psm.hs4 = (COOKIE_ECHO_SENT >> ESTABLISHED) + Pred(sctp.header_contain(sctp_cookie_ack_hdr))

    sctp.psm.sack = (ESTABLISHED >> ESTABLISHED) + Pred(sctp.header_contain(sctp_sack_hdr))
    sctp.psm.data_start = (ESTABLISHED >> MORE_FRAG) + \
        Pred(sctp.header_contain(sctp_data_hdr) & 
            (sctp.header.U == 0) & 
            (sctp.header.B == 1) & 
            (sctp.header.E == 0))
    
    sctp.psm.more_data = (MORE_FRAG >> MORE_FRAG) + \
        Pred(sctp.header_contain(sctp_data_hdr) & 
             (sctp.header.U == 0) & 
             (sctp.header.B == 0) & 
             (sctp.header.E == 0))

    sctp.psm.data_end = (MORE_FRAG >> ESTABLISHED) + \
        Pred(sctp.header_contain(sctp_data_hdr) & 
            (sctp.header.U == 0) &
            (sctp.header.B == 0) & 
            (sctp.header.E == 1))

    sctp.psm.single_frag = (ESTABLISHED >> ESTABLISHED) + \
        Pred(sctp.header_contain(sctp_data_hdr) & 
            (sctp.header.U == 0) &
            (sctp.header.B == 1) & 
            (sctp.header.E == 1))
