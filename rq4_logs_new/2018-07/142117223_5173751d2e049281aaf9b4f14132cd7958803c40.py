...
def nn_pred(model_tag, test_data):
    ...
    with open(os.path.join(data_dir, 'Input_parameter_'+model_tag+'.pickle'), 'rb') as fh:
        train_gene, input_shift, input_scale, w_hidden, w_output = pickle.load(fh)
    tf.reset_default_graph()
    global saver, graph, predition, xs, ys, x_shift, x_scale, kp
    saver = tf.train.import_meta_graph(os.path.join(data_dir, 'nn-save', model_tag, 'model.meta'))
    ...
    saver.restore(sess, os.path.join(data_dir, 'nn-save', model_tag, 'model'))
    # Testing data
