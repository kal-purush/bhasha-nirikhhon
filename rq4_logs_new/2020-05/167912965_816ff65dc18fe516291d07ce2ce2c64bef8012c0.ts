...
export async function getHubAsync(host: string) {
  const remoteId = await Hub.getRemoteId(host);
  const deviceId = (await Hub.getDevices(host, remoteId))[0].id;
  return getHub(host, remoteId, deviceId);
}