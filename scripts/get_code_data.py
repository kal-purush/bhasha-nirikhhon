...
# valid_exts = (".py", ".pyw", ".pyc", ".pyo", ".pyx", ".pxd", ".pxi", ".ipynb",  # Python
...
if __name__ == "__main__":
    log_output_dir = os.path.join("logs", f"{year}-{month:02d}")
    os.makedirs(log_output_dir, exist_ok=True)
    df = pd.read_csv(f"repos_{year}_{month:02d}.csv")
    if 'index' in df.columns:
        df = df.drop(columns=['index'])
    
    df_sample = df.sample(n=3850, random_state=42).reset_index(drop=True)  # sample more than 385
