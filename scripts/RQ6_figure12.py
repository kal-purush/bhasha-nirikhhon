...
# 1) Load
df = pd.read_csv("total_star_data.csv")
...
df = df.dropna(subset=["stargazers_count", "label", "push_bucket"])
...
sns.boxplot(data=df, x="push_bucket", y="stargazers_count", hue="label", showfliers=False)
plt.yscale("log")  # durations vary widely
plt.xlabel("GitHub Contributions Bucket", fontsize=18)
plt.ylabel("Number of Star", fontsize=18)
...
# plt.savefig("total_star_per_repo.pdf", bbox_inches='tight')
plt.show()