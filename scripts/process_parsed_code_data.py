...
def process_data(data):
    ...
    try:
        result = detector.compute_language_confidence_values_in_parallel(data)
        for i in range(len(result)):
            confidence_values = result[i]
            print(confidence_values[0]," ", data[i])
    except Exception:
        return None
...
if __name__ == "__main__":
    log_dir = f"code_parser_data/{year}-{month:02d}"
