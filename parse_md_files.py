...
def main():
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = {executor.submit(process_file, args[0], args[1]): args for args in file_folder_pairs}
        for f in tqdm(as_completed(futures), total=len(futures), desc="Processing files in parallel"):
            args = futures[f]
            try:
                langs = f.result()
                if langs:
                    # Extract date from folder name, e.g., readme_logs/2015-01
                    folder_name = os.path.basename(args[1])
                    date_str = folder_name if re.match(r"\d{4}-\d{2}", folder_name) else ""
                    # for lang in langs:
    ...
    print("NOT SUCCESSFUL====> ", not_successful)
    csv_filename = f"language_detection_{datetime.now().strftime('%Y%m%d')}.csv"
    with open(csv_filename, "w", newline='', encoding="utf-8") as csvfile:
        fieldnames = ["file", "folder", "language", "date"]
